	...
	)
{
	...
	{
		/* catalog tables: pg_partition, pg_partition_rule */
		return get_parts(relid, level, parent, inctemplate, CurrentMemoryContext, includesubparts);
	}
	GP_WRAP_END;
	return NULL;
}
	...
	)
{
	...
	GP_WRAP_START;
	{
		return FaultInjector_InjectFaultIfSet(identifier, DDLNotSpecified, (char *) "", (char *) "");
	}
	GP_WRAP_END;
...
register_mdcache_invalidation_callbacks(void)
{
	...
	for (i = 0; i < (int) lengthof(metadata_caches); i++)
	{
