...
extern "C" void exx_plannedstmt_set_flags(PlannedStmt*);
//---------------------------------------------------------------------------
//	@function:
...
CTranslatorDXLToPlStmt::SetInitPlanVariables(PlannedStmt* pplstmt)
{
    ...
    exx_plannedstmt_set_flags(pplstmt);
	...
	)
{
	...
	ForEach (plcLocation, extentry->locations)
	{
		...
		for (ULONG ul = 0; ul < (ULONG) pcdbCompDB->total_segment_dbs && !fMatchFound; ul++)
		{
...
{
	for (ULONG ul = 0; ul < (ULONG) pcdbCompDB->total_segment_dbs; ul++)
	{
		CdbComponentDatabaseInfo *pcdbCompDBInfo = &pcdbCompDB->segment_db_info[ul];
		INT iSegInd = pcdbCompDBInfo->segindex;
		if (SEGMENT_IS_ACTIVE_PRIMARY(pcdbCompDBInfo))
	...
	)
{
	...
	else if (fUsingLocation && (URI_GPFDIST == pUri->protocol || URI_GPFDISTS == pUri->protocol 
                || URI_CUSTOM == pUri->protocol
                || URI_XDRIVE == pUri->protocol
                ))
	{
	...
	)
{
	...
	if (0 < ulNumSortCols)
	{
        ...
        // 
        // XXX: FIXME: ftian: Upstream 8.3 merge left overs.
	...
	)
{
	...
	if (pagg->numCols > 0)
	{
        ...
        // XXX: ftian: PG 8.3 merge leftovers.
	...
	)
{
	...
	// XXX PG83 pwindow->partOperators = NULL;

	if (pwindow->numPartCols > 0)
	{
		pwindow->partColIdx = (AttrNumber *) gpdb::GPDBAlloc(pwindow->numPartCols * sizeof(AttrNumber));
		// XXX PG83 pwindow->partOperators = (Oid *) gpdb::GPDBAlloc(pwindow->numPartCols * sizeof(Oid));
	}
	...
	for (ULONG ul = 0; ul < ulSize; ul++)
	{
		...
		// XXX PG83 pwindowkey->nullsFirst = (bool *) gpdb::GPDBAlloc(ulNumCols * sizeof(bool));
		TranslateSortCols(pdxlnSortColList, &dxltrctxChild, pwindowkey->sortColIdx, pwindowkey->sortOperators);  // XXX PG83 , pwindowkey->nullsFirst);
	...
	)
{
    ...
    // Greenplum only support nullsLast right now.  
    psort->nullsFirst = NULL;
	// XXX PG83 psort->nullsFirst = (bool *) gpdb::GPDBAlloc(ulNumCols * sizeof(bool));

	TranslateSortCols(pdxlnSortColList, &dxltrctxChild, psort->sortColIdx, psort->sortOperators);  // XXX PG83 , psort->nullsFirst);
	...
	)
{
	...
	for (ULONG ul = 0; ul < ulArity; ul++)
	{
		...
		// XXX PG83 pboolNullsFirst[ul] = pdxlopSortCol->FSortNullsFirst();
	}
}
