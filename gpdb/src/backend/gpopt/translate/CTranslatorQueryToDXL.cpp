...
extern "C" void exx_try_to_fix_into_policy(Query *);
...
CTranslatorQueryToDXL::PdxlnCTAS()
{
	...
	for (ULONG ul = 0; ul < ulColumns; ul++)
	{
		...
		if (NULL != plColnames && ul < (ULONG) gpdb::UlListLength(plColnames))
		{
    ...
    if (m_pquery->intoPolicy == NULL) {
        exx_try_to_fix_into_policy(m_pquery);
    }
	...
	const
{
	...
	ForEach (plcSortCl, plSortCl)
	{
		...
		CDXLScalarSortCol *pdxlop = GPOS_NEW(m_pmp) CDXLScalarSortCol
                                                ...
                                                false   // XXX: FIXME: psortcl->nulls_first
												);
