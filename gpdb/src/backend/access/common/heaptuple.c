...
void
exx_heap_deform_tuple(HeapTupleHeader tup,
					  TupleDesc tupleDesc,
					  int max_natts,
					  Datum *values,
					  bool *isnull)
{
	bool		hasnulls = HeapTupleHeaderGetHasNulls(tup);
	Form_pg_attribute *att = tupleDesc->attrs;
	int			natts;			/* number of atts to extract */
	int			attnum;
	char	   *tp;				/* ptr to tuple data */
	...
	/* CK: should assert before calling this func.
	 * Assert(!is_heaptuple_memtuple(tuple)); 
	 */
	natts = HeapTupleHeaderGetNatts(tup);
	 ...
	 * the caller's arrays.
	 */
	natts = Min(natts, tupleDesc->natts);

	/* limit by max_natts */
	natts = Min(natts, max_natts);
	...
	/*
	 * If tuple doesn't have all the atts indicated by max_natts, read the
	 * rest as null
	 */
	for (; attnum < max_natts; attnum++)
	{
		values[attnum] = (Datum) 0;
		isnull[attnum] = true;
	}
