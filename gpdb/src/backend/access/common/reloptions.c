				  ...
				  ArrayBuildState *astate, bool *foundAO, bool *aovalue)
{
    ...
    if (pg_strcasecmp(SOPT_APPENDONLY, name) == 0) 
	{
		if (!parse_bool(value, &boolval))
			elog(ERROR, "invalid bool value \"%s\" for storage option \"%s\"", value, name);
	...
	else if (pg_strcasecmp(SOPT_BLOCKSIZE, name) == 0)
	{
		if (!parse_int(value, &intval, 0 /* unit flags */,
					   NULL /* hint message */))
			elog(ERROR, "invalid integer value \"%s\" for storage option \"%s\"", value, name);
	...
	if (values[4] != NULL)
	{
			...
			(result->compresslevel != 1))
		{
			if (validate)
				ereport(ERROR,
						(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
						 errmsg("compresslevel=%d is out of range for lz4 (should be 1)",
								result->compresslevel),
								  ...
								  bool co)
{
		 ...
		 pg_strcasecmp(comptype, "rle_type") == 0))
	{
			...
			(complevel != 1))
		{
				ereport(ERROR,
						(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
						 errmsg("compresslevel=%d is out of range for lz4 "
 ...
 * otherwise default for both zlib, lz4 and RLE to level 1.
 */
static int setDefaultCompressionLevel(char* compresstype)
{
	if(!compresstype || pg_strcasecmp(compresstype, "none") == 0) {
		return 0;
    } else if (pg_strcasecmp(compresstype, "zstd") == 0) {
        return 5;
    } else 
		return 1;
}
