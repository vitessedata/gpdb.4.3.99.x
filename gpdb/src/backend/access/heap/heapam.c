 ...
 * EXX, EXX_IN_PG
 */
extern bool exx_heap_getpage(HeapScanDesc scan, BlockNumber blk, void *buf);
bool exx_heap_getpage(HeapScanDesc scan, BlockNumber blk, void *buf)
{
	MIRROREDLOCK_BUFMGR_DECLARE;
    Page p;
    ...
    p = (Page) BufferGetPage(scan->rs_cbuf);
    memcpy(buf, p, BLCKSZ);
    return true;
}
