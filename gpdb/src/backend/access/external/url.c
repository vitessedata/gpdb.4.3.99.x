    ...
    if (sz > 64000) {
        ereport(ERROR,
                (errcode(ERRCODE_EXTERNAL_ROUTINE_EXCEPTION),
                 errmsg("Cannot execute external command.  Command size %d too long.", sz))); 
    }
...
url_fopen(char *url, bool forwrite, extvar_t *ev, CopyState pstate, int *response_code, const char **response_string)
{
    ...
    if (IS_XDRIVE_URI(url)) {
        return url_xdrive_fopen(url, forwrite, ev);
    }
 ...
 * command termination is not reflected until close is called.  If failOnError is
 * false, close errors are just logged.  failOnError should be false when closure
...
url_fclose(URL_FILE *file, bool failOnError, const char *relname)
{
	...
	switch (type)
	{
		...
		case CFTYPE_EXEC:
			...
			if (file->u.exec.pid == -1) {
				/* detect if the file has already been closed.
				   this is normal for exec because we need to call close as soon 
				   as the stream ends to get the rc code of the exec. This is the
				   only way to tell if the exec was successful, and must be done
				   during the read.
				*/
				ret = 0;
			}
			else if (!failOnError) {
				/* close it, ignore any error and avoid any potential proc won't die problem */
				kill(file->u.exec.pid, SIGKILL);
				pclose_with_stderr(file->u.exec.pid, file->u.exec.pipes, &sinfo);
				file->u.exec.pid = -1;
				ret = 0;
			}
			else {
			...
			else
			{
                ...
                /*
                 * EXX: EXX_IN_PG.
                 *  The following code basically assumes the executed program should exit with
                 *  code 0 when we close pipe.  This just put too much assumption on the external
                 *  program, which we do not have any control after all.  
                 *
                 *  At this stage, most likely we have read the data we wanted, we close it, ignore
                 *  whatever the external program is complaining about ...
                 *
                 */

                /*
                 * The above comment, according to CK, is totally wrong.   If exit code is not 0, we fail.
                 */
#endif 
				/*
		...
		case CFTYPE_CURL:
#ifdef USE_CURL
			{
				...
				if (file->u.curl.out.ptr)
				{
                     ...
                     * XXX 
                     *
                     * What the hell, did we do pfree here, but everywhere else it is free.
                     * But I checked, seems out.ptr indeed comes from palloc.
                     */
    ...
    switch (file->type)
    {
		...
		case CFTYPE_EXEC:
			...
			if (file->u.exec.pid == -1) {
				...
				if (want == 0) {
					/* we have EOF. For exec, we need to fclose() immediately to obtain
					   the rc code. For example, curl that failed to connect to remote
					   will return EOF immediately. Only the rc at fclose() would tell 
					   us it failed to connect. */
					StringInfoData sinfo;
					int rc;
					initStringInfo(&sinfo);
					rc = pclose_with_stderr(file->u.exec.pid, file->u.exec.pipes, &sinfo);
					file->u.exec.pid = -1;
					if (rc) {
						...
						if (rc == -1) {
							ereport( ERROR,
									 (errcode(ERRCODE_EXTERNAL_ROUTINE_EXCEPTION),
								  errmsg("cannot close external table command: %m"),
									  errdetail("command %s", url)));
						}
						else {
							char buf[512];
							ereport( ERROR,
									 (errcode(ERRCODE_EXTERNAL_ROUTINE_EXCEPTION),
									  errmsg("external table command ended with %s",
											 interpretError(rc, buf, sizeof(buf), sinfo.data, sinfo.len)),
									  errdetail("Command: %s", url)));
						}
					}
				}
        ...
        case CFTYPE_XDRIVE:
			ereport(ERROR, (errcode(ERRCODE_EXTERNAL_ROUTINE_EXCEPTION),
						errmsg("XDrive external table not supported."))); 
            break;
...
static int popen_with_stderr(int *pipes, const char *exe, bool forwrite)
{
	...
	{		
        /*
         * Ha!   Useless, but just reduce confusion when debugging forked-but-not-execed process.
         */
        MyProcPid = getpid();

        /*
         * XXX: I believe here we should reset all signal handler etc.  But I don't know what 
 ...
 */
URL_FILE*
url_xdrive_fopen(char *url, bool forwrite, extvar_t *ev)
{
	elog(ERROR, "Xdrive not suported."); 
	return 0;
}

void url_xdrive_fclose(URL_FILE *file)
{
	elog(ERROR, "Xdrive not suported."); 
}
