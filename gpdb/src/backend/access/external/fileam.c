...
static void exx_g_dataSoruce_cb(XactEvent ev, void *p)
{
    url_fclose((URL_FILE *) p, false, NULL);
}
			   ...
			   int rejLimit, bool rejLimitInRows, Oid fmterrtbl, int encoding)
{
	...
	if (fmttype_is_spq(fmtType) || fmttype_is_custom(fmtType))
	{
        char *fmtname = scan->fs_pstate->custom_formatter_name;
         ...
         * EXX: EXX_IN_PG.   
         *
         * Decide if the formatter is a deepgreen builtin.
         */
#define EXX_FMT_SPQ_IN "vitesse_spq_formatter_in"
#define EXX_FMT_SPQ_IN_ALLSEGS "vitesse_spq_formatter_in_allsegs"
        if (fmttype_is_spq(fmtType)) {
            scan->fs_formatter->fmt_user_tag = EXX_FMT_SPQ_ALLSEGS;
        } else if (fmtname) {
            if (strcmp(fmtname, EXX_FMT_SPQ_IN) == 0) {
                scan->fs_formatter->fmt_user_tag = EXX_FMT_SPQ;
            } else if (strcmp(fmtname, EXX_FMT_SPQ_IN_ALLSEGS) == 0) {
                scan->fs_formatter->fmt_user_tag = EXX_FMT_SPQ_ALLSEGS;
            }
        }
	}
...
{
    /* Close prev scan if it was already open */
    external_stopscan(scan);
    /* First call to external_getnext will reopen */
...
external_endscan(FileScanDesc scan)
{
...

	{
		 ...
		 */
		FILE* fp = scan->fs_file;
		if (!scan->fs_noop && fp) {
			scan->fs_file = NULL;
			close_external_source(fp, false, (const char*)relname);
		}
	}
...
external_insert_init(Relation rel)
{
    ...
    /*
     * EXX: EXX_IN_PG
     *  Intercept xdrive write.  Regardless if it is CSV or SPQ protocol, we catch it here.
     */
    if (IS_XDRIVE_URI(extInsertDesc->ext_uri)) {
		elog(ERROR, "Xdrive not supported.");
	}
...
external_insert(ExternalInsertDesc extInsertDesc, HeapTuple instup)
{
    ...
    /*
     * EXX: EXX_IN_PG.
     *  Intercept xdrive/spq writes.
     */
    if (extInsertDesc->ext_formatter_data && extInsertDesc->ext_formatter_data->fmt_user_tag == EXX_FMT_SPQ) {
		elog(ERROR, "XDrive/Spq not supported");
	} 
			   ...
			   bool islimitinrows, Oid fmterrtbl, int encoding)
{
	...
	/*
	 * EXX: always turn on intts for non-csv type.
	 */
	if (!fmttype_is_csv(fmtType)) {
     ...
     *
     * EXX: SPQ in in fact built in, no need for formatter func.
	 */
	if(format_is_custom && !fmttype_is_spq(fmtType)) 
	{
		Oid			procOid;
 ...
 */
void open_external_readable_source(FileScanDesc scan)
{
	extvar_t 	extvar;
	int 		response_code;
    ...
    RegisterXactCallbackOnce(exx_g_dataSoruce_cb, scan->fs_file);
}
...
open_external_writable_source(ExternalInsertDesc extInsertDesc)
{
    ...
    RegisterXactCallbackOnce(exx_g_dataSoruce_cb, extInsertDesc->ext_file);
}
...
{
	if (dataSource) {
		url_fclose((URL_FILE*) dataSource, failOnError, relname);
        UnregisterXactCallbackOnce(exx_g_dataSoruce_cb, dataSource);
	}
}
...
{
    MemoryContext oldctxt = MemoryContextSwitchTo(TopTransactionContext);
    void *p = palloc(size);
    MemoryContextSwitchTo(oldctxt);
    return p;
}
			...
			while (token)
			{
				...
				else if (pg_strcasecmp(token, "comment") == 0) 
				{
					token = strtokx2(NULL, whitespace, NULL, "'",
									nonstd_backslash, true, true, encoding);
					if (token)
						pstate->exx_comment = pstrdup(token);
					else
						goto error;
				}
