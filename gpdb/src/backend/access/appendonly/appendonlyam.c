...
/* Returns number of records processed, and exhausted flag. 
 * If exhausted, caller responsible for calling
 *  	AppendOnlyExecutionReadBlock_FinishedScanBlock(
 *													executorReadBlock);
 */
static int32_t ExxAppendOnlyExecutorReadBlock_ScanMulti(
	AppendOnlyExecutorReadBlock		*executorReadBlock,
	int32_t*                        precno,
	MemTuple*                       prec,
	int32_t*                        preclen,
    ...
    int32_t*                        exhausted)
{
	/* See AppendOnlyExecutorReadBlock_ScanNextTuple(). Note: we do not handle keys. */
	...
	*exhausted = 0;
	...
	case AoExecutorBlockKind_VarBlock:
		for (i = 0; i < nrec; ) {
			...
			itemPtr = VarBlockReaderGetNextItemPtr(&executorReadBlock->varBlockReader,
												   &itemLen);
			if (itemPtr == NULL) {
				*exhausted = 1;
				break;
			...
			if (itemLen > 0) {
				precno[i] = (executorReadBlock->blockFirstRowNum
							 + executorReadBlock->currentItemCount - 1);
				prec[i] = (MemTuple) itemPtr;
				preclen[i] = itemLen;
				i++;
			}
	...
	case AoExecutorBlockKind_SingleRow:
		{
			...
			precno[0] = executorReadBlock->blockFirstRowNum;
			prec[0] = (MemTuple) executorReadBlock->singleRow;
			preclen[0] = executorReadBlock->singleRowLen;
...
extern int32_t exx_appendonly_scan_multi(AppendOnlyScanDesc scan,
								  int32_t*  precno,
								  MemTuple* prec,
								  int32_t*  preclen,
								  int32_t   nrec,
...
/* 
 * Scan and return up to nrec tuples in precno[], prec[] and preclen[]. 
 * Also, return flag in *check_alignment if caller should check alignment of the 
 * returned records.
 * Also, if a tuple is not visible to current snapshot, set 
 * its (recno, ptr, reclen) to 0.
 */
int32_t exx_appendonly_scan_multi(AppendOnlyScanDesc scan,
								  int32_t*  precno,
								  MemTuple* prec,
								  int32_t*  preclen,
								  int32_t   nrec,
								  bool*     check_alignment)
{
	/* see appendonly_getnext() */
	const bool isSnapshotAny = (scan->snapshot == SnapshotAny);
	int ret_nrec = 0;
	...
	while (ret_nrec == 0) {
		...
		int32_t exhausted = 0;
		AppendOnlyExecutorReadBlock* block = &scan->executorReadBlock;
		ret_nrec = ExxAppendOnlyExecutorReadBlock_ScanMulti(block,
															precno,
															prec,
															preclen,
															nrec,
		 ...
		 * problematic
		 */
		*check_alignment = ! IsAOBlockAndMemtupleAlignmentFixed(block->storageRead->storageAttributes.version);
			
		if (!isSnapshotAny) {
			AppendOnlyVisimap *visimap = &scan->visibilityMap;
			for (int i = 0; i < ret_nrec; i++) {
				AOTupleId tid;
				AOTupleIdInit_Init(&tid);
				AOTupleIdInit_segmentFileNum(&tid, scan->executorReadBlock.segmentFileNum);
				AOTupleIdInit_rowNum(&tid, precno[i]);
				if (!AppendOnlyVisimap_IsVisible(visimap, &tid)) {
					/* tuple is invisible */
					precno[i] = 0;
					preclen[i] = 0;
					prec[i] = 0;
		...
		if (exhausted) {
			AppendOnlyExecutionReadBlock_FinishedScanBlock(&scan->executorReadBlock);
			scan->bufferDone = true;
		}
	}
