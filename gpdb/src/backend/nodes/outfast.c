...
_outScanInfo(StringInfo str, Scan *node)
{
	...
	else
	{
        ...
        /*
         * EXX: code was rte->type != RTE_SUBQUERY, which is obviously non-sense.
         */
...
_outPlannedStmt(StringInfo str, PlannedStmt *node)
{
     ...
     * EXX EXX_IN_PG
     */
    WRITE_INT_FIELD(exxExecMode);
	WRITE_INT_FIELD(exxSpillZ);
    WRITE_INT_FIELD(exxStackTrace);
    WRITE_INT_FIELD(exxDbgBreakMask);
...
_outMaterial(StringInfo str, Material *node)
{
    ...
    WRITE_INT64_FIELD(exx_share_counter);
...
static void 
_outSample(StringInfo str, Sample *node)
{
    WRITE_NODE_TYPE("SAMPLE");
    _outPlanInfo(str, (Plan *) node);
    WRITE_ENUM_FIELD(sampleMethod, SampleMethod);
    WRITE_NODE_FIELD(sampleRate);
    WRITE_NODE_FIELD(sampleCount);
}

static void
_outTransducer(StringInfo str, Transducer *node)
{
    WRITE_NODE_TYPE("TRANSDUCER");
    _outPlanInfo(str, (Plan *) node);
    WRITE_INT_FIELD(nOutCol);
    WRITE_INT_FIELD(nInCol);
    WRITE_NODE_FIELD(tr_args);
}
...
_outMotion(StringInfo str, Motion *node)
{
    ...
    WRITE_BOOL_FIELD(exx_binary_mode);
...
{
    WRITE_NODE_TYPE("SETDISTRIBUTION");
...
_outQuery(StringInfo str, Query *node)
{
    ...
    WRITE_NODE_FIELD(sampleRateQry);
    WRITE_NODE_FIELD(sampleCountQry);
	WRITE_NODE_FIELD(rowMarks);
