...
_copyPlannedStmt(PlannedStmt *from)
{
    ...
    /* EXX EXX_IN_PG */
    COPY_SCALAR_FIELD(exxExecMode);
    COPY_SCALAR_FIELD(exxSpillZ); 
    COPY_SCALAR_FIELD(exxStackTrace);
    COPY_SCALAR_FIELD(exxDbgBreakMask);
...
CopyScanFields(Scan *from, Scan *newnode)
{
    ...
    COPY_SCALAR_FIELD(exx_part_oid);
}
...
_copyShareInputScan(ShareInputScan *from)
{
    ...
    COPY_SCALAR_FIELD(exx_share_counter);
	return newnode;
}
...
static Sample*
_copySample(Sample *from)
{
    Sample *newnode = makeNode(Sample);
    CopyPlanFields((Plan *) from, (Plan *) newnode);

    COPY_SCALAR_FIELD(sampleMethod);
    COPY_NODE_FIELD(sampleRate);
    COPY_NODE_FIELD(sampleCount);
    return newnode;
}

static Transducer*
_copyTransducer(Transducer *from)
{
    Transducer *newnode = makeNode(Transducer);
    CopyPlanFields((Plan *) from, (Plan *) newnode);
    COPY_SCALAR_FIELD(nOutCol);
    COPY_SCALAR_FIELD(nInCol);
    COPY_NODE_FIELD(tr_args);
    return newnode; 
}
...
_copyMotion(Motion *from)
{
    ...
    COPY_SCALAR_FIELD(exx_binary_mode);
...
_copyQuery(Query *from)
{
	...
	COPY_NODE_FIELD(limitOffset);
	COPY_NODE_FIELD(limitCount);
    COPY_NODE_FIELD(sampleRateQry);
    COPY_NODE_FIELD(sampleCountQry);
	COPY_NODE_FIELD(rowMarks);
...
copyObject(void *from)
{
	...
	switch (nodeTag(from))
	{
		...
		case T_Limit:
			...
			break;
        case T_Sample:
            retval = _copySample(from);
            break;
        case T_Transducer:
            retval = _copyTransducer(from);
            break;
		case T_Motion:
