...
_readQuery(const char ** str)
{
    ...
    READ_NODE_FIELD(sampleRateQry);
    READ_NODE_FIELD(sampleCountQry);
...
_readPlannedStmt(const char ** str)
{
    ...
    /* EXX EXX_IN_PG */
    READ_INT_FIELD(exxExecMode);
    READ_INT_FIELD(exxSpillZ);
    READ_INT_FIELD(exxStackTrace); 
    READ_INT_FIELD(exxDbgBreakMask); 
...
_readMaterial(const char ** str)
{
    ...
    READ_INT64_FIELD(exx_share_counter);
...
static Sample *
_readSample(const char **str)
{
    READ_LOCALS(Sample);
    readPlanInfo(str, (Plan *) local_node);

    READ_ENUM_FIELD(sampleMethod, SampleMethod);
    READ_NODE_FIELD(sampleRate);
    READ_NODE_FIELD(sampleCount);
    READ_DONE();
}

static Transducer*
_readTransducer(const char ** str)
{
    READ_LOCALS(Transducer);
    readPlanInfo(str, (Plan *)local_node);

    READ_INT_FIELD(nOutCol);
    READ_INT_FIELD(nInCol);
    READ_NODE_FIELD(tr_args);
    READ_DONE();
}
...
_readMotion(const char ** str)
{
    ...
    READ_BOOL_FIELD(exx_binary_mode);
...
readNodeBinary(const char ** str)
{
	...
	switch(nt)
	{
			...
			case T_Limit:
				...
				break;
            case T_Sample:
                return_value = _readSample(str);
                break;
            case T_Transducer:
                return_value = _readTransducer(str);
                break;
			case T_Hash:
