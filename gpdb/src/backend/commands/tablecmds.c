	...
	if (!superuser() && Gp_role == GP_ROLE_DISPATCH)
	{
		...
		else
		{
			...
			else if(!gp_external_grant_privileges)
			{
				...
				if ( (uri->protocol == URI_GPFDIST || uri->protocol == URI_GPFDISTS || uri->protocol == URI_XDRIVE) 
                        && iswritable)
				{
				...
				else if ( (uri->protocol == URI_GPFDIST || uri->protocol == URI_GPFDISTS || uri->protocol == URI_XDRIVE) && !iswritable)
				{
			   ...
			   DropStmt *stmt, char relkind)
{
    ...
    if (!OidIsValid(relOid)) {
        /*
         * Missed to find the object to drop.  Drop with "if exists"
         * just notify the same, unlock as not performing any operation
         * and return back to convey didn't drop the relation.
...
static char transformFormatType(char *formatname)
{
	...
	if(pg_strcasecmp(formatname, "text") == 0)
        ...
        result = 'p';
    else if(pg_strcasecmp(formatname, "spq") == 0)
        result = 's';
	else
		ereport(ERROR,
				(errcode(ERRCODE_SYNTAX_ERROR),
...
static Datum transformFormatOpts(char formattype, List *formatOpts, int numcols, bool iswritable)
{
	...
	if (fmttype_is_text(formattype) || fmttype_is_csv(formattype))
	{
		...
		/* EXX: */ 
		/* integer timestamp */
		len += exx_intts ? strlen(" integer timestamp") : 0;
		/* comment '#' */
		len += 7 + 1 + 1 + 1 + 1 + 1; 
		...
		else if (fmttype_is_csv(formattype))
		{
			...
			if (exx_comment) {
				if (*exx_comment == '\'') {
					sprintf(fmt_comment, " comment ''''");
				} else {
					sprintf(fmt_comment, " comment '%c'", *exx_comment);
				}
			}
			
			sprintf((char *) format_str, "delimiter '%s' null '%s' escape '%s' quote '%s'%s%s%s%s%s%s%s",
					...
					(eol_str ?  nl.data : ""),
					(exx_intts ? " integer timestamp" : ""),
					(exx_comment ? fmt_comment : "")
					);
		}
		else
		{
    ...
    else if (fmttype_is_spq(formattype)) 
    {
        /*
         * While spq is implemented as a custom formatter, we give it special treatment so that
         * we do not need to hack a formatter function.
         */
        char *val = NULL;
        if (iswritable) {
            val = "xdrive_export";
