...
void ValidateControlChars(bool copy, bool load, bool csv_mode, char *delim,
						...
						int num_columns,
						bool exx_intts, char *exx_comment)
{
	...
	if (!csv_mode && exx_intts) {
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				errmsg("integer timestamp can only be used in csv mode.")));
	} 

#if 0
	NYI
	if (!csv_mode && exx_comment) {
#else
	if (exx_comment) {
#endif
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				errmsg("comment char not implemented yet."))); 
	}
}
...
DoCopyInternal(const CopyStmt *stmt, const char *queryString, CopyState cstate)
{
	...
	foreach(option, stmt->options)
	{
		...
		else if (strcmp(defel->defname, "using_intts") == 0) 
		{
			...
			cstate->exx_intts = intVal(defel->arg);
		}
		else if (strcmp(defel->defname, "comment") == 0) 
		{
#if 0
			if (cstate->exx_comment) {
#else 
			if (true) {
#endif
				ereport(ERROR,
			...
			cstate->exx_comment = strVal(defel->arg);
		}
		else
	...
	ValidateControlChars(true,
						 ...
						 cstate->exx_intts, cstate->exx_comment
						 );
										  ...
										  Form_pg_attribute *attr)
{
	...
	if (cstate->csv_mode)
	{
		...
		/* EXX: */
		if (cstate->exx_intts) {
			appendStringInfo(cdbcopy_cmd, " INTEGER TIMESTAMP");
		} 
		if (cstate->exx_comment) {
			appendStringInfo(cdbcopy_cmd, " COMMENT AS E'%s'", escape_quotes(cstate->exx_comment));
		}
	}
		...
		if (bytesread > 0 || !cstate->fe_eof)
		{
			...
			while (!cstate->raw_buf_done)
			{
				...
				for (i = 0; i < part_p_nattrs; i++)
				{
					...
					if (!nulls[h_attnum - 1])
						cdbhash(part_hash, h_key, p_attr_types[h_attnum - 1], NULL);
					else
...
CopyFrom(CopyState cstate)
{
     ...
     * there maybe duplicate free in ExecDropSingleTupleTableSlot; if not, they
					  ...
					  int num_phys_attrs, Form_pg_attribute *attr)
{
	...
	if (cstate->exx_intts || cstate->exx_comment) {
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				 errmsg("USING INTEGER TIMESTAMP and COMMENT are only supported by xdrive.")));
	}
