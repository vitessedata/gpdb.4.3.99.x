					 ...
					 double totalrows)
{
	...
	SelectSortFunction(mystats->ltopr, &cmpFn, &cmpSk); 
...
compare_scalars(const void *a, const void *b, void *arg)
{
	...
	compare = ApplySortFunction(cxt->cmpFn, cxt->cmpSk, /* cxt->cmpFlags, */
								da, false, db, false);
	if (compare != 0)
		return compare;
