...
init_tuplestore_state(ShareInputScanState *node)
{
	...
	if(share_type == SHARE_MATERIAL_XSLICE)
	{
		...
		shareinput_create_bufname_prefix(rwfile_prefix, sizeof(rwfile_prefix), sisc->share_id, sisc->exx_share_counter);
...
ExecSliceDependencyShareInputScan(ShareInputScanState *node)
{
	...
	elog(DEBUG1, "SISC READER (shareid=%d, slice=%d): exec dependency on slice %d.", 
			sisc->share_id, currentSliceId, currentSliceId); 
    ...
    if (sisc->share_type == SHARE_MATERIAL_XSLICE 
		...
		ShareNodeEntry *snEntry = ExecGetShareNodeEntry(estate, sisc->share_id, false);
        Assert(snEntry && snEntry->sharePlan);
        bool waitready = false;
        if (sisc->share_type == SHARE_MATERIAL_XSLICE) {
            Material *m = (Material *) snEntry->sharePlan; 
            if (m->driver_slice != currentSliceId) {
                waitready = true;
            }
        } else {
            Sort *s = (Sort *) snEntry->sharePlan; 
            if (s->driver_slice != currentSliceId) {
                waitready = true;
            }
        }

        if (waitready) {
            node->share_lk_ctxt = shareinput_reader_waitready(sisc->share_id, sisc->exx_share_counter,
...
void ExecEndShareInputScan(ShareInputScanState *node)
{
	...
	if(node->share_lk_ctxt)
		shareinput_reader_notifydone(node->share_lk_ctxt, sisc->share_id, sisc->exx_share_counter);
...
void shareinput_create_bufname_prefix(char* p, int size, int share_id, int64_t share_counter)
{
	snprintf(p, size, "%s_SIRW_%d_%d_%ld", 
            PG_TEMP_FILE_PREFIX, gp_session_id, share_id, share_counter);
}
 ...
 */
static void sisc_lockname(char* p, int size, int share_id, int64_t share_counter, const char* name)
{
	if (snprintf(p, size,
			"%s/%s/%s_gpcdb2.sisc_%d_%d_%d_%ld_%s",
			getCurrentTempFilePath, PG_TEMP_FILES_DIR, PG_TEMP_FILE_PREFIX, 
			Gp_segment, gp_session_id, share_id, share_counter, name
			) > size)
	{
		ereport(ERROR, (errmsg("cannot generate path %s/%s/%s_gpcdb2.sisc_%d_%d_%d_%ld_%s",
                        getCurrentTempFilePath, PG_TEMP_FILES_DIR, PG_TEMP_FILE_PREFIX,
                        Gp_segment, gp_session_id, share_id, share_counter, name)));
	}
}
 ...
 */
void *
shareinput_reader_waitready(int share_id, int64_t sc, PlanGenerator planGen)
{
	mpp_fd_set rset;
	struct timeval tval;
	int n;
	...
	sisc_lockname(pctxt->lkname_ready, MAXPGPATH, share_id, sc, "ready");
    elog(DEBUG1, "Shareinput wait ready on %s.", pctxt->lkname_ready);
	...
	sisc_lockname(pctxt->lkname_done, MAXPGPATH, share_id, sc, "done");
 ...
 */
void *
shareinput_writer_notifyready(int share_id, int64_t sc, int xslice, PlanGenerator planGen)
{
	int n;
	...
	sisc_lockname(pctxt->lkname_ready, MAXPGPATH, share_id, sc, "ready");
    elog(DEBUG1, "Shareinput notify ready on %s xsclie count %d.", pctxt->lkname_ready, xslice);
 ...
 */
void
shareinput_reader_notifydone(void *ctxt, int share_id, int64_t sc)
{
 ...
 */
void
shareinput_writer_waitdone(void *ctxt, int share_id, int64_t sc, int nsharer_xslice)
{
	ShareInput_Lk_Context *pctxt = (ShareInput_Lk_Context *) ctxt;
	mpp_fd_set rset;
	struct timeval tval;
	...
	while(ack_needed > 0 && !IsAbortInProgress())
	{
		CHECK_FOR_INTERRUPTS();
