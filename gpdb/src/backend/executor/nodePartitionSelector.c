...
ExecPartitionSelector(PartitionSelectorState *node)
{
	...
	{
        if (!node->exx_static_partition_propagation_called) {
            ...
            *node->acceptedLeafPart = NULL;
            node->exx_static_partition_propagation_called = true;
        }
		return NULL;
	}
	/* elog(LOG, "in ExecPartitionSelector, %d", ps->staticSelection); */
...
partition_propagation(List *partOids, List *scanIds, int32 selectorId)
{
	...
	/*elog(LOG, "propagating %d parts", partOids->length);*/
	...
	forboth (lcOid, partOids, lcScanId, scanIds)
	{
		...
		/* elog(LOG, "... propagate %d %d", partOid, scanId);*/
