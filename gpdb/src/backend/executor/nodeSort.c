...
ExecSort(SortState *node)
{
	...
	if (!node->sort_Done)
	{
		...
		if(plannode->share_type == SHARE_SORT_XSLICE)
		{
			...
			shareinput_create_bufname_prefix(rwfile_prefix, sizeof(rwfile_prefix), plannode->share_id, plannode->exx_share_counter);
		...
		if(plannode->share_type != SHARE_NOTSHARED) 
		{
			...
			if(plannode->share_type == SHARE_SORT_XSLICE)
			{
				if(plannode->driver_slice == currentSliceId)
				{
					...
					node->share_lk_ctxt = shareinput_writer_notifyready(plannode->share_id, plannode->exx_share_counter, plannode->nsharer_xslice,
...
ExecEagerFreeSort(SortState *node)
{
	...
	if (NULL != node->tuplesortstate->sortstore || NULL != node->tuplesortstate->sortstore_mk)
	{
		...
		{
			shareinput_writer_waitdone(node->share_lk_ctxt, sort->share_id, sort->exx_share_counter, sort->nsharer_xslice);
		}
