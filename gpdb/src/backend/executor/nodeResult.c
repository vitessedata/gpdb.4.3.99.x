	...
	if (resultNode->hashFilter)
	{
		...
		foreach(cell, resultNode->hashList)
		{
			...
			if (!isnull)
			{
				...
				cdbhash(hash, hAttr, att_type, NULL);
			}
			else
				cdbhashnull(hash);
		}
...
/* Expose for EXX */
extern bool exx_result_hash_filter(Result *resultNode, TupleTableSlot *resultSlot);
bool exx_result_hash_filter(Result *resultNode, TupleTableSlot *resultSlot)
{
    return TupleMatchesHashFilter(resultNode, resultSlot);
}
