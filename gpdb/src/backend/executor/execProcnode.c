...
ExecSliceDependencyNode(PlanState *node)
{
    ...
    if (nodeTag(node) == T_MotionState) {
        ...
        if (ms->mstype != MOTIONSTATE_SEND) {
            elog(DEBUG1, "Exec skip a motion recv, current slice id %d, mstype %d", currentSliceId, ms->mstype);
            return;
        } else {
            elog(DEBUG1, "Exec process a motion send, current slice id %d, mstype %d", currentSliceId, ms->mstype);
        }
    }

	if(nodeTag(node) == T_ShareInputScanState) {
        elog(DEBUG1, "Exec process a share inputscan slice id %d", currentSliceId); 
...
Exec_Jmp_Sample:
Exec_Jmp_Transducer:
	elog(ERROR, "Sample and Transducer not supported");
	goto Exec_Jmp_Done;
