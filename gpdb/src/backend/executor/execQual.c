...
/* EXX_IN_PG */
extern bool exx_ScalarArrayOpUseFastPath(ScalarArrayOpExprState *sstate);
bool exx_ScalarArrayOpUseFastPath(ScalarArrayOpExprState *sstate)
{
	return sstate->fxprstate.xprstate.evalfunc == (ExprStateEvalFunc) ExecEvalFPScalarArrayInt
		|| sstate->fxprstate.xprstate.evalfunc == (ExprStateEvalFunc) ExecEvalFPScalarArrayStr
		;
}
			 ...
			 bool *isNull, ExprDoneCond *isDone)
{
	...
	if (caseExpr->arg)
	{
        /*
         * EXX:
         *  The following was from upstream Fix nested case expression.
         *  exx.enable does not have the bug.
         *
         *  The comment and code are totally non-sense.  But whatever, it 
         *  worked.
         */

        /*
         * caseValue_datum and caseValue_isNull from the econtext store the rusults of case
         * expression.  caseValue_isNull will be true if the caseValue_datum store null value.
         * Both caseValue_datum and caseValue_isNull should change at the same time and they shouldn't
         * go out of sync.
         * Hence pass the temporary variable(caseValue_isNull) and once evaluation is done, 
 ...
 * EXX: Expose. 
 */
extern void exx_part_oid(PartOidExprState* exprstate, int64_t *datum, bool *isnull);
void exx_part_oid(PartOidExprState* exprstate, int64_t *datum, bool *isnull)
{
    *datum = ExecEvalPartOidExpr(exprstate, NULL, isnull, NULL);
}
