...
static void
ExecHashTableExplainBatches(HashJoinTableStats *stats, 
                            StringInfo      buf,
                            int             ibatch_begin,
                            int             ibatch_end,
                            const char     *title);
 ...
 */
HashJoinTableStats* 
ExecHashTableExplainInit(HashState *hashState, HashJoinState *hjstate, int nbatch)
{
    MemoryContext   oldcxt;
    HashJoinTableStats *stats = NULL;
    ...
    stats = (HashJoinTableStats *)palloc0(sizeof(*stats));
    stats->endedbatch = -1;

    /* Create per-batch statistics array. */
    stats->batchstats =
        (HashJoinBatchStats *)palloc0(nbatch * sizeof(stats->batchstats[0]));
...
ExecHashTableExplainEnd(PlanState *planstate, struct StringInfoData *buf)
{
    ...
    if (hashtable) {
        ...
        stats = hjstate->exx_hj_stats;
    }

    if (!jinstrument || !stats) { 
        return;
    }
	...
	if (hashtable && !hashtable->eagerlyReleased)
	{		
    ...
    if (hashtable) {
        ...
        if (hashtable->nbatch > 1)
        {
            ...
            {
                ExecHashTableExplainBatches(stats, buf, 0, 1, "Initial");
                ExecHashTableExplainBatches(stats,
                        buf,
                        1,
            ...
            {
                ExecHashTableExplainBatches(stats, buf, 0, hashtable->nbatch, "Reuse");
            }
        }
        ...
        /* EXX: EXX_IN_PG */
        uint64 workmemwanted = 0;
        for (int i = 0; i < stats->nbatchstats; i++) {
            workmemwanted += stats->batchstats[i].innerfilesize;
        }
        jinstrument->workmemwanted = Max(jinstrument->workmemwanted, workmemwanted);

        ExecHashTableExplainBatches(stats, buf, 0, stats->nbatchstats, "HashJoin Overflow"); 

        /* Report hash chain statistics. */
        if (stats->nonemptybatches > 0) {
            appendStringInfo(buf,
                    "Hash chain length"
                    " %.1f avg, %.0f max, using %d buckets.  ",
                    cdbexplain_agg_avg(&stats->chainlength),
                    stats->chainlength.vmax,
                    stats->chainlength.vcnt); 
            appendStringInfoChar(buf, '\n');
                            ...
                            const char     *title)
{
    ...
    Assert(ibatch_begin >= 0 && ibatch_begin <= ibatch_end &&
           ibatch_end <= stats->nbatchstats &&
