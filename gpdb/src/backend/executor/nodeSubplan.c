...
ExecInitSubPlan(SubPlanState *node, EState *estate, int eflags)
{
    ...
    if (sp_estate->es_instrument) {
        sp_estate->exx_jit = (ExxJitInstrumentation *) palloc0(sizeof(ExxJitInstrumentation));
    } else {
        sp_estate->exx_jit = 0;
    }
...
SubplanQueryDesc(QueryDesc * qd)
{
    ...
    /* EXX EXX_IN_PG */
    substmt->exxExecMode = stmt->exxExecMode;
    substmt->exxStackTrace = stmt->exxStackTrace;
    substmt->exxDbgBreakMask = stmt->exxDbgBreakMask;
	substmt->exxSpillZ = stmt->exxSpillZ;
