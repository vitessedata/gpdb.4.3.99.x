...
ExecMaterial(MaterialState *node)
{
	...
	if (ts == NULL && (ma->share_type != SHARE_NOTSHARED || node->randomAccess))
	{
		...
		if(ma->share_type == SHARE_MATERIAL_XSLICE)
		{
			...
			shareinput_create_bufname_prefix(rwfile_prefix, sizeof(rwfile_prefix), ma->share_id, ma->exx_share_counter);
		...
		if(ma->share_type != SHARE_NOTSHARED)
		{
			...
			if (ma->share_type == SHARE_MATERIAL_XSLICE)
			{
				if (ma->driver_slice == currentSliceId)
				{
					...
					node->share_lk_ctxt = shareinput_writer_notifyready(ma->share_id, ma->exx_share_counter, ma->nsharer_xslice,
...
ExecInitMaterial(Material *node, EState *estate, int eflags)
{
    ...
    matstate->exx_spq_file = NULL;
    matstate->exx_done = false;
...
ExecEndMaterial(MaterialState *node)
{
    ...
    if (node->exx_spq_file != NULL) {
		Assert(!"Material cannot use spq.");
    } else {
        /*
        ...
        if (node->ts_state->matstore != NULL)
        {
            ...
            {
                shareinput_writer_waitdone(node->share_lk_ctxt, ma->share_id, ma->exx_share_counter, ma->nsharer_xslice);
            }
            Assert(node->ts_pos);
