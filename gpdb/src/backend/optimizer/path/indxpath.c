	...
	foreach(l, clauses)
	{
		...
		foreach(j, ((BoolExpr *) rinfo->orclause)->args)
		{
			...
			else
			{
                ...
                /* 
                 * EXX: BUG BUG: Hit this assert.  
                 * Assert(!restriction_is_or_clause((RestrictInfo *) orarg));
                 *
                 * This is due to the change with exx_tune_plan.  See initsplan.c.  
                 * After the tune plan hack, this is indeed possible, for the orarg to be an or clause (Huh? good variable names.)
                 *
                 * I believe, the push down is a much more important case.   So we keep exx_tune_plan to true by default, and
                 * check, bail out here.
                 *
                 * Or, we should flatten the caluse somehow?   Need an optimizer guy ...
                 */
                if(restriction_is_or_clause((RestrictInfo *) orarg)) {
                    indlist = NIL;
                } else {
                    indlist = find_usable_indexes(root, rel,
											  list_make1(orarg),
