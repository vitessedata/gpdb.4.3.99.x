				  ...
				  int flags)
{
    ...
    if (walker(query->sampleRateQry, context)) {
        return true;
    }
    if (walker(query->sampleCountQry, context)) {
        return true;
    }
				 ...
				 void *context)
{
	...
	switch (nodeTag(node))
	{
        ...
        case T_Sample:
			if (walk_plan_node_fields((Plan *) node, walker, context))
				return true;
			if (walker((Node *) (((Sample*) node)->sampleRate), context))
                return true;
			if (walker((Node *) (((Sample*) node)->sampleCount), context))
                return true;
			/* Other fields are simple items and lists of simple items. */
			break;
