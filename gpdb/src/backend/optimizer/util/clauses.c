				...
				eval_const_expressions_context *context)
{
	...
	if (!IsA(querytree, Query) ||
        ...
        querytree->sampleRateQry ||
        querytree->sampleCountQry ||
				   ...
				   int flags)
{
    ...
    MUTATE(query->sampleRateQry, query->sampleRateQry, Node *);
    MUTATE(query->sampleCountQry, query->sampleCountQry, Node *);
...
flatten_join_alias_var_optimizer(Query *query, int queryLevel)
{
    ...
    Node *sampleRateQry = queryNew->sampleRateQry;
    if (sampleRateQry != NULL) {
        queryNew->sampleRateQry = flatten_join_alias_vars(root, sampleRateQry);
        pfree(sampleRateQry);
    }

    Node *sampleCountQry = queryNew->sampleCountQry; 
    if (sampleCountQry != NULL) {
        queryNew->sampleCountQry = flatten_join_alias_vars(root, sampleCountQry); 
        pfree(sampleCountQry); 
    }
