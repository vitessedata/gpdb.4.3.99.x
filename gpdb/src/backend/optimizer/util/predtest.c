...
ConstHashTableHash(const void *keyPtr, Size keysize)
{
	...
	if ( c->constisnull)
	{
		return exx_datum_hashnull(0);
	}
	else
	{
		exx_datum_hashfn_t* fn = exx_get_datum_hashfn(c->consttype);
		return fn(c->constvalue, 0);
	}
}
...
{
	Assert( exx_get_datum_hashfn(valueToCopy->consttype));
...
TryProcessEqualityNodeForPossibleValues(OpExpr *expr, Node *variable, PossibleValueSet *resultOut )
{
	...
	{
		if ( exx_get_datum_hashfn(constExpr->consttype))
		{
			/**
	...
	else
	{
		...
		else
		{
			...
			Assert( exx_get_datum_hashfn(consttype));
