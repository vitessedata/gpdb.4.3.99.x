						...
						List **postponed_qual_list)
{
	...
	switch (bms_membership(relids))
	{
		...
		case BMS_MULTIPLE:
            if (0) { 
                /*
                 * EXX: EXX_IN_PG:    Here is the place that we derive additional predicates for
                 * push down.
                 */
                 ...
                 *
                 * TPCH Q7:
                 *      select * from A, B where (A.x = 1 and B.y = 2) or (A.x = 3 and B.y = 4);
                 *
                 ...
                 *      AND (B.y = 2 OR B.y = 4);
                 *
                 * Note that in the rewrite, we have two predicates that can be pushed down.
                 * See make_sub_restrictinfos for how to pull out the innards ...
                 */

                if (or_clause((Node *) clause)) {
                    typedef struct Or_Clause {
                        Relids rids; 
                        int round;
                        List *exprs;
                    } Or_Clause;

                    int nrel = bms_num_members(relids);
                    int nround = list_length(((BoolExpr *) clause)->args);
                    ListCell *tmp;
                    ...
                    memset(orc, 0, sizeof(Or_Clause) * nrel);
                    int curround = 0;

                    foreach(tmp, ((BoolExpr *) clause)->args) {
                        Expr* term = (Expr *) lfirst(tmp);
                        Relids termrids = pull_varnos((Node *)term); 
                        BMS_Membership mbshp = bms_membership(termrids);
                        curround++;

                        if (mbshp == BMS_SINGLETON) {
                            for (int i = 0; i < nrel; i++) {
                                if (orc[i].rids == 0) {
                                    orc[i].rids = termrids;
                                    if (orc[i].round == (curround - 1)) {
                                        orc[i].round = curround;
                                    }
                                    orc[i].exprs = lappend(orc[i].exprs, term);
                                    break;
                                } else if (bms_equal(orc[i].rids, termrids)) {
                                    if (orc[i].round == (curround - 1)) {
                                        orc[i].round = curround;
                                    }
                                    orc[i].exprs = lappend(orc[i].exprs, term);
                        ...
                        } else if (and_clause((Node *) term)) {
                            ListCell *atmp; 
                            foreach(atmp, ((BoolExpr *) term)->args) {
                                Expr* aterm = (Expr *) lfirst(atmp);
                                Relids atermrids = pull_varnos((Node *) aterm); 
                                BMS_Membership ambshp = bms_membership(atermrids);
                                if (ambshp == BMS_SINGLETON) {
                                    for (int i = 0; i < nrel; i++) {
                                        if (orc[i].rids == 0) {
                                            orc[i].rids = atermrids;
                                            if (orc[i].round == (curround - 1)) {
                                                orc[i].round = curround;
                                            }
                                            orc[i].exprs = lappend(orc[i].exprs, aterm);
                                            break;
                                        } else if (bms_equal(orc[i].rids, atermrids)) {
                                            if (orc[i].round == (curround - 1)) {
                                                orc[i].round = curround;
                                                orc[i].exprs = lappend(orc[i].exprs, aterm);
                                            }
                    ...
                    for (int i = 0; i < nrel; i++) {
                        if (orc[i].round == nround) {
                            Expr *e = make_orclause(orc[i].exprs);
                            RestrictInfo *ri = make_restrictinfo(e, 
									 is_pushed_down,
									 outerjoin_delayed,
									 pseudoconstant,
                                     orc[i].rids);

                            rel = find_base_rel(root, bms_singleton_member(orc[i].rids)); 
                            if (!is_deduced || is_deduced_but_not_equijoin || 
                                    !qual_is_redundant(root, ri, rel->baserestrictinfo)) { 
