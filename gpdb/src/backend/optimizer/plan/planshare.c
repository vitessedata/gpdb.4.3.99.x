...
int64_t get_plan_exx_share_counter(Plan *p)
{
	if(IsA(p, Material))
		return ((Material *) p)->exx_share_counter; 
	
	if(IsA(p, Sort))
		return ((Sort *) p)->exx_share_counter; 

	Assert(IsA(p, ShareInputScan));
	return ((ShareInputScan *) p)->exx_share_counter; 
}

void set_plan_exx_share_counter(Plan *p, int64_t exx_share_counter) 
{
	if(IsA(p, Material))
		((Material *) p)->exx_share_counter = exx_share_counter; 
	else if(IsA(p, Sort))
		((Sort *) p)->exx_share_counter = exx_share_counter; 
	else
	{
		Assert(IsA(p, ShareInputScan));
		((ShareInputScan *) p)->exx_share_counter = exx_share_counter; 
	}
}
...
static ShareInputScan *make_shareinputscan(PlannerInfo *root, Plan *inputplan) 
{
    ...
    set_plan_exx_share_counter((Plan *) sisc, get_plan_exx_share_counter(inputplan));
	sisc->driver_slice = -1;
