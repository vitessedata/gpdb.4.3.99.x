...
static void disuse_physical_tlist(Plan *plan, Path *path, List *addlist);
 ...
 */
static void
disuse_physical_tlist(Plan *plan, Path *path, List *addlist)
{
	...
	switch (path->pathtype)
	{
		...
		case T_ValuesScan:
            ...
            if (addlist) {
				plan->targetlist = add_to_flat_tlist(plan->targetlist, addlist, true /* resjunk */);
            }
...
create_material_plan(PlannerInfo *root, MaterialPath *best_path)
{
	...
	disuse_physical_tlist(subplan, best_path->subpath, 0);
...
create_motion_plan(PlannerInfo *root, CdbMotionPath *path)
{
     ...
     *
     * EXX: Bug fix.  The expression used in motion/hash is also "used"
     * therefore must be projected.
     */
    List *addlist = NIL;
    if (CdbPathLocus_IsHashed(path->path.locus)) { 
        ListCell *partkeycell;
        foreach (partkeycell, path->path.locus.partkey) {
            List *pathkey = (List *) lfirst(partkeycell);
            ListCell *pathkeycell;

            /*
             * NB: EXX
             *
             * At this point, we will have an expression (inside a PathKeyItem)
             * for hash motion.  The pathkey, is a cannonical representation, or,
             * equi_key_list of a PathKeyItem, so we have to loop over the list
             * and choose one that is within the range of relids.   This is root
             * cause of a very difficult bug.
             */
            bool found = false;
            foreach(pathkeycell, pathkey) {
                PathKeyItem *item = (PathKeyItem *) lfirst(pathkeycell);
                if (!bms_is_subset(item->cdb_key_relids, path->path.parent->relids)) {
                    continue;
                } else {
                    Expr *key = (Expr *) item->key;
                    addlist = lappend(addlist, key);
                    found = true;
                    break;
                }
            }
            ...
            if (!found) {
                elog(LOG, "An expr in pathkey is out of relids range.");
            }
        }
    }
    disuse_physical_tlist(subplan, subpath, addlist); 
						 ...
						 List *tlist, List *scan_clauses)
{
							   ...
							   uri->protocol == URI_CUSTOM))
	{
		...
		{
            /*
             * XDRIVE FALLS HERE:  Use all available segments.
             */
					  ...
					  Plan *inner_plan)
{
	...
	{
		disuse_physical_tlist(outer_plan, best_path->jpath.outerjoinpath, 0);
		sort =
	...
	{
		disuse_physical_tlist(inner_plan, best_path->jpath.innerjoinpath, 0);
		sort =
			make_sort_from_pathkeys(root,
									inner_plan,
...
Sample *
make_sample(Plan *lefttree, Node *sampleRate, Node *sampleCount, SampleMethod m) 
{
    Sample *sample = makeNode(Sample);
    Plan *plan = &sample->plan;

    sample->sampleMethod = m;
    sample->sampleRate = copyObject(sampleRate);
    sample->sampleCount = copyObject(sampleCount);

    if (m == SAMPLE_RATE) {
        Assert( sampleRate && (!sampleCount));
        Assert( IsA(sampleRate, Const));

        Const *cr = (Const *) sampleRate;
        Assert( !cr->constisnull && cr->consttype == FLOAT8OID);
        double r = DatumGetFloat8(cr->constvalue);

        if (r < 0) {
            ereport(ERROR,
                    (errcode(ERRCODE_INVALID_LIMIT_VALUE), 
                     errmsg("LIMIT SAMPLE cannot sample negative percentage.")));
        }

        copy_plan_costsize(plan, lefttree);
        plan->plan_rows *= (r / 100.0);
        plan->targetlist = cdbpullup_targetlist(lefttree, 
                cdbpullup_exprHasSubplanRef((Expr *) lefttree->targetlist));
        plan->qual = NIL;
        plan->lefttree = lefttree;
        ...
        /* This is the right flow.  After rate sample, flow is preserved. */
        plan->flow = pull_up_Flow(plan, plan->lefttree, false);
    } else {
        Assert( !sampleRate && sampleCount);
        Assert( IsA(sampleCount, Const));
        Const *cc = (Const *) sampleCount; 
        Assert( !cc->constisnull && cc->consttype == INT8OID); 
        int64_t c = DatumGetInt64(cc->constvalue);

        if (c <= 0) {
            ereport(ERROR,
                    (errcode(ERRCODE_INVALID_LIMIT_VALUE), 
                     errmsg("LIMIT SAMPLE cannot sample zero or negative ROWS."))); 
        }
        ...
        /* 
         * build target list.   Notice that it is really fake, because between two
         * stages and the motion, we communicate via binary protocol.
         */
        plan->targetlist = cdbpullup_targetlist(lefttree, 
                cdbpullup_exprHasSubplanRef((Expr *) lefttree->targetlist));
        ...
        /*
         * flow is also fake -- but this is OK, as either it is gathered, then ok,
         * or we are in distributed stage 1, we will add a motion to gather it.
         */
        plan->flow = pull_up_Flow(plan, plan->lefttree, false);
    }
