...
#include "cdb/cdbdisp.h"    /* remove_subquery_in_RTEs */
...
static void postprocess_plan(PlannedStmt *plan)
{
    ...
    /*
     * To save memory, and on network bandwitch, when the plan is dispatched QEs, strip 
     * all subquery RTEs of the original query objects.
     */
...
static PlannedStmt* 
run_orca(Query* parse, int cursorOptions, ParamListInfo boundParams, bool force)
{
		...
		ParamListInfo boundParams)
{
    ...
    result = run_orca(parse, cursorOptions, boundParams, false);
	...
	if (!result)
	{
        ...
        PG_CATCH();
        {
            if (geterrcode() == ERRCODE_PLANNER_NOPLAN) {
                /* 
                 * Oh, the planner so messed up and we would let ORCA to resque.
                 */
                result = run_orca(parse, cursorOptions, boundParams, true);
            } else {
                PG_RE_THROW();
            }
        }
    ...
    /*
     * Some plans are just bad.   Let's fix it.  Hope Orca can help ...
     */
    if (0) { 
        PlannedStmt *fix = NULL;

        /*
         * Case 1: Postgres init plan crap is FUBAR.  
         */
        if (result->nCrossLevelParams > 0) {
            fix = run_orca(parse, cursorOptions, boundParams, true);
        }

        /* 
         * Fix plan, if Orca cannot produce a plan, well, we keep on running PG plan.
         * Hopefully it will error out somewhere instead of crashing ...
         */
        if (fix) {
            result = fix;
        } 
...
void exx_plannedstmt_set_flags(PlannedStmt *result)
{
} 
...
standard_planner(Query *parse, int cursorOptions, ParamListInfo boundParams)
{
    ...
    exx_plannedstmt_set_flags(result);
				 ...
				 PlannerConfig *config)
{
	...
	parse->sampleRateQry = preprocess_expression(root, parse->sampleRateQry,
											  EXPRKIND_LIMIT);
	parse->sampleCountQry = preprocess_expression(root, parse->sampleCountQry, 
											  EXPRKIND_LIMIT);
...
grouping_planner(PlannerInfo *root, double tuple_fraction)
{
			...
			if ( root->config->gp_enable_preunique && needMotion )
			{
				...
				if ( alt_cost < base_cost || root->config->gp_eager_preunique )
				{
					...
					if (parse->sortClause)
					{
                        ...
                        /*
                         * Update numOrderbyCols to the length of sort_pathkeys.
                         * For cdb: decide with sort attribute should be preserved by merge gather motion.
                         */
    ...
    if (parse->sampleRateQry || parse->sampleCountQry) {
        if (parse->sampleRateQry) {
            /* Rate sampling, no need for gathering. */
            result_plan = (Plan *) make_sample(result_plan, 
                    parse->sampleRateQry, parse->sampleCountQry, SAMPLE_RATE);
        } else {
            /* 
             * Count sampling, two stage.   Stage 1.
             */
            result_plan = (Plan *) make_sample(result_plan, 
                    parse->sampleRateQry, parse->sampleCountQry, SAMPLE_COUNT_STAGE_1);

            /*
             * If we have a distributed flow, need to gather.
             */
            if (Gp_role == GP_ROLE_DISPATCH && result_plan->flow->flotype == FLOW_PARTITIONED) {
             ...
             */
            result_plan = (Plan *) make_sample(result_plan, 
                    parse->sampleRateQry, parse->sampleCountQry, SAMPLE_COUNT_STAGE_2);
        }
    }
