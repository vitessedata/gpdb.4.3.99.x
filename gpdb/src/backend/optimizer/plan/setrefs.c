...
set_plan_refs(PlannerGlobal *glob, Plan *plan, int rtoffset)
{
	...
	switch (nodeTag(plan))
	{
        ...
        case T_Sample:
            {
                Sample *splan = (Sample *) plan;
				set_dummy_tlist_references(plan, rtoffset, false);
				Assert(splan->plan.qual == NIL);

				splan->sampleRate = 
					fix_scan_expr(glob, splan->sampleRate, rtoffset);
				splan->sampleCount = 
					fix_scan_expr(glob, splan->sampleCount, rtoffset);
            }
            break;
	
        case T_Transducer:
            {
                Transducer *tr = (Transducer *) plan;
				set_dummy_tlist_references(plan, rtoffset, false);
				Assert(tr->plan.qual == NIL);
            }
            break;
