...
#define COPYARRAY(dest,src,lenfld,datfld) \
	...
	switch (nodeTag(node))
	{
		...
		case T_DynamicTableScan:
			{
                ...
                newTableScan->exx_part_oid = tableScan->exx_part_oid;
				return (Node *) newTableScan;
			}
			break;
        ...
        case T_Sample:
            {
				...
				FLATCOPY(news, s, Sample); 
				PLANMUTATE(news, s); 
				MUTATE(news->sampleCount, s->sampleCount, Node *); 
				MUTATE(news->sampleRate, s->sampleRate, Node *); 
                return (Node *) news;
            }
            break;
        ...
        case T_Transducer:
            {
                Transducer *tr = (Transducer *) node;
                Transducer *newTr;

                FLATCOPY(newTr, tr, Transducer);
                PLANMUTATE(newTr, tr);
                return (Node *) newTr;
            }
            break;
