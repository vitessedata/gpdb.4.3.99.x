...
bool exx_path_has_valid_hash_relids(Path *path) 
{
    if (path->parent && path->parent->relids) {
        return exx_locus_has_valid_hash_relids(path->locus, path->parent->relids);
    }
    return true;
}

bool exx_locus_has_valid_hash_relids(CdbPathLocus locus, Relids relids) 
{
    if (CdbPathLocus_IsHashed(locus)) { 
        ListCell *partkeycell;
        foreach (partkeycell, locus.partkey) {
