...
static void hashDatum(Datum datum, Oid type, FmgrInfo *hfn, CdbHash* cdbhash);
static void hashNullDatum(CdbHash* cdbhash);
 ...
 * 2 - mask for reducing hash value to segment id. only used when 
 *     #segments is power of 2. When it is not, we just do modulo.
 *
...
makeCdbHash(int numsegs)
{
	 ...
	 */
	h->mask = ispowof2(numsegs) ? numsegs - 1 : 0;
	...
	ereport(DEBUG4,
	  (errmsg("CDBHASH started for %d segment databases",
			  h->numsegs)));
 ...
 */
void
cdbhash(CdbHash *h, Datum datum, Oid type, FmgrInfo *hfn)
{
	hashDatum(datum, type, hfn, h);
}

/*
 * Add an attribute to the hash calculation.
 * **IMPORTANT: any new hard coded support for a data type in here
 * must be added to isGreenplumDbHashable() below! It must also 
 * be added to exx_reset_datum_hashfn().
 *
 *
 ...
 */
static void
hashDatum(Datum datum, Oid type, FmgrInfo *hfn, CdbHash *cdbhash)
{
	...
	switch (type)
	{
		...
		default:
			{
				if (hfn == NULL || !OidIsValid(hfn->fn_oid)) {
					ereport(ERROR,
							(errcode(ERRCODE_CDB_FEATURE_NOT_YET),
							 errmsg("Type %u is not hashable.", type)));
				} else {
					intbuf = (int64) FunctionCall1(hfn, datum);
					buf = &intbuf;
					len = sizeof(intbuf);
				}
				break;
	...
	cdbhash->hash = fnv1_32_buf(buf, len, cdbhash->hash);
	if(tofree)
		pfree(tofree);
}
...
bool isGreenplumDbHashable(Oid typid)
{
	...
	switch(typid)
	{
		...
		default:
			{
				bool canhash;
				Operator optup = equality_oper(typid, true);
				if (!optup) {
					return false;
				}
				canhash = ((Form_pg_operator) GETSTRUCT(optup))->oprcanhash;
...
fnv1_32_buf(void *buf, size_t len, uint32 hval)
{
	...
	while (bp < be)
	{
		...
		hval *= FNV_32_PRIME;
		error out please ... save some worries on this case
#else
