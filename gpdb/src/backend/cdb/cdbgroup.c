...
join_dqa_coplan(PlannerInfo *root, MppGroupContext *ctx, Plan *outer, int dqa_index)
{
	...
	{
        /*
         * Flag to indicate if all group cols are hashable.  I believe we
         * can do hashjoin as long as there are some columns are hashable,
         * but for now, be dumb.
         */
        bool hashable = true;
		...
		for ( attrno = 1; attrno <= ctx->numGroupCols; attrno++ )
		{
			...
			if ( ctx->join_strategy == DqaJoinHash )
			{
                /* 
                 * EXX: EXX_IN_PG 
                 * BUGFIX: make sure the OpExpr is actually hashable.
                 */
                OpExpr* opexpr = (OpExpr *) qual;
                Oid hashfn = get_op_hash_function(opexpr->opno);
                if (hashfn == InvalidOid) {
                    hashable = false;
                } else {
		...
		if ( hashable && ctx->join_strategy == DqaJoinHash )
		{
			/* Make the hash join. */
