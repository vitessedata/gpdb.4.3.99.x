...
static bool exx_has_valid_relids(List *keys, Relids relids)
{
    ListCell* cell;
    foreach (cell, keys) {
        PathKeyItem *item = (PathKeyItem *) lfirst(cell); 
                                    ...
                                    ListCell                   *partkeycell)
{
    ...
    /*
     * EXX: EXX Bug fix.  Take a look at how the colocus partkey is constructed.
     *
     * Colocus partkey is contructred out of the mergeclause, it bascally assign
     * the locus to the colocus, so they can colocate.   rinfo has two parts, 
     * left and right -- I don't think the following code make any sense, but the
     * point is the push one to the colocus.   The following magical code may
     * put a partkey (therefore Expr) to colocus but the expr references Var from
     * the other side.   Later, when create the motion, we hit the nasty but because
     * the wrong side cannot resolve the hash expr.
     *
     * Too afraid to fix the logic, so I will at the end, check veriry relids and
     * swap copathkey to other.   Nasty.
     *
     * See createplan.c, cdbpull.c for the fix. 
     */
    if (!copathkey)
    {
        foreach(rcell, ctx->mergeclause_list)
        {
            ...
            if (CdbPathLocus_IsHashed(ctx->locus))
            {
                ...
                if (sublist == rinfo->left_pathkey) {
                    ...
                    other = rinfo->left_pathkey;
                } else if (sublist == rinfo->right_pathkey) {
                    copathkey = rinfo->left_pathkey;
                    other = rinfo->right_pathkey;
                }

                if (copathkey) {
                    if (!exx_has_valid_relids(copathkey, ctx->relids)) {
                        copathkey = other;
                    }
                }
            }
