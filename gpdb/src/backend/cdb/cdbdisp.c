	...
	if (node == NULL)
	{
 	...
 	if (IsA(node, RangeTblEntry))
 	{
 		...
 		if (RTE_SUBQUERY == rte->rtekind && NULL != rte->subquery)
 		{
             ...
             *
             * XXX: We could save a lot more memory by deep freeing the many fields in the Query too, but 
             * I am not sure which of the them might be shared by other projects in the tree.
 			 */
            pfree(rte->subquery);
