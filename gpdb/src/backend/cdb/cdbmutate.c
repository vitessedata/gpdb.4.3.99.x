	...
	for (i = 0; i < targetPolicy->nattrs; i++)
	{
		...
		foreach(cell, plan->targetlist)
		{
			...
			else
			{
				...
				cdbhash(h, c->constvalue, typeIsArrayType(c->consttype) ? ANYARRAYOID : c->consttype, NULL);
			}
...
apply_motion(PlannerInfo *root, Plan *plan, Query *query)
{
    ...
    query->sampleRateQry = NULL;
    query->sampleCountQry = NULL;
...
static int64_t exx_counter_next()
{
	static int64_t next;
	return next++;
}
...
static bool shareinput_mutator_dag_to_tree(Node *node, ApplyShareInputContext* ctxt, bool fPop)
{
	...
	if(IsA(plan, ShareInputScan))
	{
		...
		if(list_member_ptr(ctxt->sharedNodes, subplan))
		{
            ...
            set_plan_exx_share_counter(plan, get_plan_exx_share_counter(subplan));
			plan->lefttree = NULL;
			return false;
		}
		else
		{
            ...
            set_plan_exx_share_counter(subplan, exx_counter_next());
			set_plan_share_id(plan, get_plan_share_id(subplan));
            set_plan_exx_share_counter(plan, get_plan_exx_share_counter(subplan)); 
...
/*
 * Second pass on ShareInput xslice.  It marks teh hsared node xslices,
...
/* 
 * The fourth pass.  If a shareinput is running on QD, then all slices in this share 
 * must be on QD.  Move them to QD.
 */
static bool shareinput_mutator_xslice_4(Node *node, ApplyShareInputContext *ctxt, bool fPop)
{
	Plan *plan = (Plan *) node;
	int motId = shareinput_peekmot(ctxt);
...
Plan *apply_shareinput_xslice(Plan *plan, PlannerGlobal *glob)
{
     ...
     * is in the main plan, or vice versa.  First pass, collect all producers.
     */
    ...
    /*
     * Now walk again, process all consumers.
     */
	foreach (lp, glob->subplans)
	{
		Plan	   *subplan = (Plan *) lfirst(lp);
...
cdbhash_const(Const *pconst, int iSegments)
{
	...
	else
	{
		...
		cdbhash(pcdbhash, pconst->constvalue, oidType, NULL);
	}		
                                           ...
                                           PlannerInfo *root)
{
	...
	switch (((Node*)(plan))->type)
	{
        ...
        case T_Sample:
        {
			...
			find_params_walker((Node *) sample->sampleRate, &context);
			find_params_walker((Node *) sample->sampleCount, &context);
            break;
        }
        case T_Transducer:
        {
	...
	if (NIL != plan->initPlan)
	{
		...
		foreach (lc, plan->initPlan)
		{
			...
			if (1 != list_length(initplan->setParam))
			{
                /*
                 * XXX: NOTE: This restriction, or bug, has been fixed in upstream.  Unfortunately
                 * it is based on some merge work, which we are not willing to take at this moment.
                 *
                 * See upstream 9926d5e3.
                 */
				ereport(ERROR, (
								errcode(ERRCODE_CDB_INTERNAL_ERROR),
								errmsg("unexpected param list length != 1") ));
			}
...
extern void exx_try_to_fix_into_policy(Query *query);
void exx_try_to_fix_into_policy(Query *query)
{
    // User does not spec a distributed by clause.  
    GpPolicy			*targetPolicy = NULL;
				
    targetPolicy = (GpPolicy *) palloc0(sizeof(GpPolicy)- sizeof(targetPolicy->attrs) + sizeof(targetPolicy->attrs[0]));
    targetPolicy->nattrs = 0;
     ...
     */
    int i;
    for (i = 0; i < list_length(query->targetList); i++)
    {
        TargetEntry *target = 
            get_tle_by_resno(query->targetList, i + 1);
    ...
    if (targetPolicy->nattrs == 0) {
        ...
        for (i=0;i<targetPolicy->nattrs;i++)
        {
            TargetEntry *target = get_tle_by_resno(query->targetList, targetPolicy->attrs[i]);
            if (i>0)
                strcat(columns, ", ");
            if (target->resname)
                strcat(columns, target->resname);
