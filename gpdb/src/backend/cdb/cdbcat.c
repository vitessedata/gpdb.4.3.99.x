...
GpPolicyFetch(MemoryContext mcxt, Oid tbloid)
{
	...
	/*
	 * Skip if qExec or utility mode.   Fake a trivial one.
	 */
	if (Gp_role != GP_ROLE_DISPATCH) {
        policy = (GpPolicy *) MemoryContextAlloc(mcxt, sizeof(GpPolicy)); 
    ...
    if (get_rel_relstorage(tbloid) == RELSTORAGE_EXTERNAL) {
         ...
         * into the relcache, before pg_exttable entry has been created.
