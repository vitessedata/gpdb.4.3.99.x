 ...
 * ----------------------------------------------------------------
 */
extern int32 eval_propagation_expression(PartitionSelectorState *node);
int32 eval_propagation_expression(PartitionSelectorState *node)
{
 ...
 * ----------------------------------------------------------------
 */
extern bool eval_part_qual(ExprState *exprstate, PartitionSelectorState *node, TupleTableSlot *inputTuple);
bool eval_part_qual(ExprState *exprstate, PartitionSelectorState *node, TupleTableSlot *inputTuple)
{
	...
	List *qualList = list_make1(exprstate); /* CK: qualList is a leak! */
...
partition_selection(PartitionNode *pn, PartitionAccessMethods *accessMethods, Oid root_oid, Datum value, Oid exprTypid, bool isNull)
{
	...
	/* get_next_level_matched_partition will do a binary search */
...
processLevel(PartitionSelectorState *node, int level, TupleTableSlot *inputTuple)
{
	...
	if (NULL != equalityPredicate)
	{
		...
		/* elog(LOG, "level %d/%d equal pred", level, ps->nLevels); */
	...
	{
		/* elog(LOG, "level %d/%d gen pred", level, ps->nLevels); */
	...
	else
	{
		 ...
		 */
		/* elog(LOG, "level %d/%d none pred", level, ps->nLevels); */
	...
	foreach (lc, satisfiedPartConstraints)
	{
		...
		if (level == ps->nLevels - 1)
		{
			...
			/* elog(LOG, "leaf: lo %ld  hi %ld",
				 partConstraint->lowerBound ? partConstraint->lowerBound->constvalue : -999,
				 partConstraint->upperBound ? partConstraint->lowerBound->constvalue : -999); */
			...
			if (shouldPropagate)
			{
				if (NULL != ps->propagationExpression)
				{
					if (!list_member_oid(selparts->partOids, partConstraint->pRule->parchildrelid))
					{
						...
						int scanId = eval_propagation_expression(node);
...
initPartitionSelection(bool isRunTime, PartitionSelector *node, EState *estate)
{
    ...
    psstate->exx_static_partition_propagation_called = false;
