...
extern int exx_explain_verbosity; 
...
static void exx_merge_jitinfo(ExxJitInstrumentation *ji, ExxJitInstrumentation *ji2)
{
    int nextslot = 0;
    for (; ji->jitinfo[nextslot].oid != 0 && nextslot < EXX_JIT_INFO_MAX; nextslot++) ;

    for (int i = 0; i < EXX_JIT_INFO_MAX; i++) {
        /* All merged */
        if (ji2->jitinfo[i].oid == 0) {
            return;
        }

        /* No space left */
        if (nextslot == EXX_JIT_INFO_MAX) {
            return;
        }

        bool ins = true;
        for (int j = 0; j < nextslot; j++) {
            if (ji->jitinfo[j].oid == ji2->jitinfo[i].oid) {
                ins = false;
                break;
            }
        }
        ...
        if (ins) { 
            ji->jitinfo[nextslot] = ji2->jitinfo[i];
            nextslot++;
        }
    }
}
                             ...
                             CdbExplain_SliceWorker    *out_worker)
{
    ...
    exx_merge_jitinfo(&out_worker->exx_jit, estate->exx_jit);
}                               /* cdbexplain_collectSliceStats */
                             ...
                             CdbExplain_RecvStatCtx    *recvstatctx)
{
    ...
    *ssw = hdr->worker;
     ...
     *  Rollup jit info.
     */
    exx_merge_jitinfo(&ss->exx_jit, &hdr->worker.exx_jit);
...
cdbexplain_depositStatsToNode(PlanState *planstate, CdbExplain_RecvStatCtx *ctx)
{
    ...
    ExxJitInstrumentation       exx_jit_instr;
    memset(&exx_jit_instr, 0, sizeof(exx_jit_instr));
    ...
    for (imsgptr = 0; imsgptr < ctx->nmsgptr; imsgptr++)
    {
        ...
        *nsi = *rsi;
         ...
         * EXX EXX_IN_PG:
         */
        exx_merge_jitinfo(&exx_jit_instr, &rsh->worker.exx_jit);
    }
