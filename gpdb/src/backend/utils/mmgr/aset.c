...
Pg_AllocSetContextCreate(MemoryContext parent,
					  const char *name,
					  Size minContextSize,
					  Size initBlockSize,
					  Size maxBlockSize)
...
{
    /*
     * exx_pool and exx_mxpool does not have the infrastructure to be the parent context of 
     * another memory context.   Note that exx master 936 handles this correctly, but it 
     * is a rather heavy mechanism.   Here, we basically says if it is a exx_pool or mxpool,
     * just bail out to per query context.
     */
    if (parent) {
        if (parent->type == T_MPoolContext || parent->type == T_MxPoolContext) {
			elog(ERROR, "Should not use MPOOL");
        }
    } 

    return Pg_AllocSetContextCreate(parent, name, minContextSize, initBlockSize, maxBlockSize);
}
