...
volatile QueryspaceDesc *queryEntry = NULL;
 ...
 */
volatile int32 querySpaceNestingLevel = 0;
...
/*
 * Decrement the number of workfiles created. This should be called when
 * closing and deleting a workfile.
 */
void
WorkfileQueryspace_SubtractWorkfile(int32 nFiles)
{
	if (NULL == queryEntry)
	{
		/*
...
WorkfileQueryspace_ReleaseEntry(void)
{
    ...
    QueryspaceDesc *saveEntry = queryEntry;

    /*
     * Set queryEntry to NULL as we may soon CHECK_FOR_INTERRUPTS, which can come back to this
     * method if an ERROR cleanup (e.g. because of a QueryCancelPending) invokes ExecutorEnd.
     * We don't wnat to release our queryEntry again.
     */
    queryEntry = NULL;

	bool deleted = SyncHTRelease(queryspace_Hashtable, saveEntry); 
	if (deleted)
	{
