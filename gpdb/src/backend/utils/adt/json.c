...
#if 0
        /* EXX: EXX_IN_PG: We use UDT/UDF for json, therefore, there is no JSONOID. */
		case JSONOID:
 ...
 */
Datum
json_parse_valid(PG_FUNCTION_ARGS)
{
	text	   *t = PG_GETARG_TEXT_PP(0);
    text       *result = t;
	JsonLexContext *lex;
...
json_is_valid(PG_FUNCTION_ARGS)
{
    ...
    PG_TRY();
    {
        lex = makeJsonLexContext(t, false);
        pg_parse_json(lex, &nullSemAction);
    }
    PG_CATCH();
    {
