...
/*
 * EXX IN PG:
 *  This GUC implements modified utf-8, see comments below.   This is a GUC,
 *  but due to genius of postgres guys, wchar.c is symlinked to interface/libpq/wchar.c
 *  and then, compiled and linked to libpq.   If we put the definition of this
 *  guc in its usual places like cdbvars.c, exx_runtime.cpp, etc, it will cause
 *  link failure for all the utilities -- thank god actually the utils are linking
 *  libpq, otherwise, we may ship a broken libpq unnoticed.   Whew.
 */
extern bool exx_use_modified_utf8; 
bool exx_use_modified_utf8 = false;
...
pg_utf8_islegal(const unsigned char *source, int length)
{
    ...
    if (exx_use_modified_utf8 && length == 2) {
        /*
         * So called modified UTF-8.  Code point U0000 is encoded as two bytes
         * 11000000 10000000, or 0xC080.
         */
        if (source[0] == 0xC0 && source[1] == 0x80) {
            return true;
        }
    }
...
const char* exx_utf8_handle_zero_str(const char *s, int *len)
{
    ...
    int len2 = *len * 2;
    char* ret = palloc(len2 + 1);
    len2 = 0;
    for (int i = 0; i < *len; i++) {
        if (s[i] == 0) {
            /* change s[i] from 0x00 to two bytes sequence 11000000 10000000 */
            ret[len2++] = 0xC0; 
            ret[len2++] = 0x80;
        } else {
            ret[len2++] = s[i];
...
/*
 * Verify a utf8 string is valid.   Return 0 on invalid string.
 *
 * If guc says we should handle modified utf-8, do conversion if necessary 
 * and return palloced string if conversion is indeed needed.
 */
const char*
pg_verifymbstr_modified_utf8(const char *mbstr, int *len, bool noError)
{
    int encoding = GetDatabaseEncoding();
    if (encoding == PG_UTF8 && exx_use_modified_utf8) {
        mbstr = exx_utf8_handle_zero_str(mbstr, len);
    }

    if (pg_verify_mbstr_len(encoding, mbstr, *len, noError) >= 0) {
        return mbstr;
    }
    return 0;
}
