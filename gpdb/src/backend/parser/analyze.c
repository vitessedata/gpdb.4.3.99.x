...
/* Hook for plugins to get control at the end of parse analysis */
...
static List *do_parse_analyze(Node *parseTree, ParseState *pstate, post_parse_analyze_hook_type hook);
			  ...
			  Oid *paramTypes, int numParams)
{
	...
	result = do_parse_analyze(parseTree, pstate, post_parse_analyze_hook);
...
parse_sub_analyze(Node *parseTree, ParseState *parentParseState)
{
	...
	result = do_parse_analyze(parseTree, pstate, NULL);
 ...
 */
static List *
do_parse_analyze(Node *parseTree, ParseState *pstate, post_parse_analyze_hook_type hook)
{
	List	   *result = NIL;
...
transformGroupedWindows(Query *qry)
{
    ...
    subq->sampleRateQry = NULL;
    subq->sampleCountQry = NULL;
	subq->rowMarks = NIL;
...
transformSelectStmt(ParseState *pstate, SelectStmt *stmt)
{
	...
	qry->limitCount = transformLimitClause(pstate, stmt->limitCount,
										   "LIMIT");
    qry->sampleRateQry = transformLimitClause(pstate, stmt->sampleRate, "SAMPLERATE");
    qry->sampleCountQry = transformLimitClause(pstate, stmt->sampleCount, "SAMPLECOUNT");
...
transformSetOperationStmt(ParseState *pstate, SelectStmt *stmt)
{
    ...
    sampleRate = stmt->sampleRate;
    sampleCount = stmt->sampleCount;
	lockingClause = stmt->lockingClause;

	stmt->sortClause = NIL;
	stmt->limitOffset = NULL;
	stmt->limitCount = NULL;
    stmt->sampleRate = NULL;
    stmt->sampleCount = NULL;
    ...
    qry->sampleRateQry = transformLimitClause(pstate, sampleRate, "SAMPLERATE");
    qry->sampleCountQry = transformLimitClause(pstate, sampleCount, "SAMPLECOUNT");
...
isSetopLeaf(SelectStmt *stmt)
{
	...
	{
		Assert(stmt->larg != NULL && stmt->rarg != NULL);
		if (stmt->sortClause || stmt->limitOffset || stmt->limitCount || stmt->sampleRate || stmt->sampleCount ||
			stmt->lockingClause)
			return true;
		else
			return false;
