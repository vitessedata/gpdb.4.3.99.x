...
static void insertSelectOptions(SelectStmt *stmt,
                                ...
                                Node *sampleRate, Node *sampleCount,
...
%type <list>	stmtblock stmtmulti
				...
				group_clause group_elem_list group_elem TriggerFuncArgs select_limit select_sample
...
%token <keyword> ABORT_P ABSOLUTE_P ACCESS ACTION ACTIVE ADD_P ADMIN AFTER
	...
	SAMPLE SAVEPOINT SCATTER SCHEMA SCROLL SEARCH SECOND_P 
...
RenameStmt: ALTER AGGREGATE func_name aggr_args RENAME TO name
			...
			| ALTER EXTERNAL TABLE relation_expr RENAME TO name
				{
					RenameStmt *n = makeNode(RenameStmt);
					n->renameType = OBJECT_EXTTABLE; 
					n->relation = $4;
					n->subname = NULL;
					n->newname = $7;
					$$ = (Node *)n;
...
AlterObjectSchemaStmt:
			...
			| ALTER EXTERNAL TABLE relation_expr SET SCHEMA name
				{
					AlterObjectSchemaStmt *n = makeNode(AlterObjectSchemaStmt);
					n->objectType = OBJECT_EXTTABLE;
					n->relation = $4;
					n->newschema = $7;
					$$ = (Node *)n;
				}
			...
			| select_clause opt_sort_clause for_locking_clause opt_select_limit
				{
					insertSelectOptions((SelectStmt *) $1, $2, $3,
										list_nth($4, 0), list_nth($4, 1), list_nth($4, 2), list_nth($4, 3),
										NULL);
					$$ = $1;
				}
			| select_clause opt_sort_clause select_limit opt_for_locking_clause
				{
					insertSelectOptions((SelectStmt *) $1, $2, $4,
										list_nth($3, 0), list_nth($3, 1), list_nth($3, 2), list_nth($3, 3),
										NULL);
					$$ = $1;
				}
			...
			| with_clause select_clause opt_sort_clause for_locking_clause opt_select_limit
				{
					insertSelectOptions((SelectStmt *) $2, $3, $4,
										list_nth($5, 0), list_nth($5, 1), list_nth($5, 2), list_nth($5, 3),
										$1);
					$$ = $2;
				}
...
select_limit:
			...
			| FETCH first_or_next opt_select_fetch_first_value row_or_rows ONLY
				{ $$ = list_make4(NULL, $3, NULL, NULL); }
			| OFFSET select_offset_value2 row_or_rows FETCH first_or_next opt_select_fetch_first_value row_or_rows ONLY
				{ $$ = list_make4($2, $6, NULL, NULL); }
            | LIMIT SAMPLE select_sample 
                { 
                    Node *rate = (Node *) list_nth($3, 0);
                    Node *cnt = (Node *) list_nth($3, 1);
                    $$ = list_make4(NULL, NULL, rate, cnt); 
                }
		;

select_sample: Iconst ROWS 
             { 
                Node *n = makeIntConst($1, @1); 
                $$ = list_make2(NULL, n);
             }
             | NumericOnly PERCENT 
             { 
                Node *n = makeAConst($1, @1);
                $$ = list_make2(n, NULL);
             }
             ;
					...
					WithClause *withClause)
{
    ...
    if (sampleRate || sampleCount) {
        Assert(sampleRate != NULL || sampleCount != NULL);
        Assert(sampleRate == NULL || sampleCount == NULL);
        if (stmt->limitCount || stmt->limitOffset || stmt->sortClause || stmt->sampleRate || stmt->sampleCount) {
            ereport(ERROR, (errcode(ERRCODE_SYNTAX_ERROR),
                     errmsg("syntax error at or near \"SAMPLE\""),
                     sampleRate != NULL ? scanner_errposition(exprLocation((Node *) sampleRate)) 
                                        : scanner_errposition(exprLocation((Node *) sampleCount))
                                        )); 
        }
        stmt->sampleRate = sampleRate;
        stmt->sampleCount = sampleCount;
    }
