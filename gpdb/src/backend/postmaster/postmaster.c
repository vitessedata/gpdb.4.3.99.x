	...
	if (ListenAddresses)
	{
		...
		foreach(l, elemlist)
		{
			...
			if (status == STATUS_OK)
				...
				ereport(WARNING,
						(errmsg("could not create listen socket for \"%s\", port %d",
								curhost, PostPortNumber)));
		}
...
PostmasterRandom(void)
{
	...
	if (random_seed == 0)
	{
		...
		srandom(random_seed + MyProcPid);
        srand48(random_seed + MyProcPid); 
	}
