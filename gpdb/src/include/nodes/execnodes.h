...
typedef struct EState
{
     ...
     *  Instrumentation.
     */
    struct ExxJitInstrumentation *exx_jit;
} EState;
...
typedef struct HashJoinState
{
         ...
         *  Stats
         */
        struct HashJoinTableStats *exx_hj_stats;
} HashJoinState;
...
typedef struct exx_spq_file_cursor_t {
    int64_t offset;
    int32_t ntup;
    int32_t idx;
} exx_spq_file_cursor_t;
...
typedef struct MaterialState
{
        ...
        /*
         * When running in exx mode -- that is, if this is a shared_xslice material, and exxExecMode is set.
         * We do not put it in GenericTupStore, so that not to mess up old greenplum code.
         */
        void    *exx_spq_file;  
        bool    exx_done;
} MaterialState;
...
typedef struct ShareInputScanState
{
     ...
     */
    void    *exx_spq_file;
    bool    exx_need_call_next;
    exx_spq_file_cursor_t exx_spq_markpos;
} ShareInputScanState;

/* XXX Should move into buf file */
extern void *shareinput_reader_waitready(int share_id, int64_t share_count, PlanGenerator planGen);
extern void *shareinput_writer_notifyready(int share_id, int64_t share_count, int nsharer_xslice_notify_ready, PlanGenerator planGen);
extern void shareinput_reader_notifydone(void *, int share_id, int64_t share_count);
extern void shareinput_writer_waitdone(void *, int share_id, int64_t share_count, int nsharer_xslice_wait_done);
extern void shareinput_create_bufname_prefix(char* p, int size, int share_id, int64_t share_count);
extern void exx_shareinput_create_fn(char *p, int size, int share_id, int64_t share_counter);
...
typedef struct TransducerApiCtxt
{
    struct TransducerApi *api;
...
typedef struct AggState
{
    ...
    /*
     * EXX:
     *  Agg tranducer.   If transducer has an order by inside agg fun, a mk tuplesort.
     */
    struct exx_transducer_info_t {
        int trAggNo;
        Oid *colOids;

        int state;
        struct TupleTableSlot *nextSlot;
        Datum *dbuf;
        bool *nbuf;
...
{
	PlanState	ps;				/* its first field is NodeTag */
    double      rate;           /* PERCENT case: percent */
    int64       count;          /* ROWS case: number of rows */

	/* ROWS case */
    bool        all_added;      /* Flags that sampler is fully loaded */
    int64       curr;           /* Number of rows returned from sampler. */
    int64       curr_cnt;       /* Number of rows saved in sampler */
    void        **saved_tuples; /* The sampler: an array of MemTuple* */
    double      total;          /* Number of input rows;  yes, double is more convenient. */

	/* ROWS case stage 2 */
    struct CdbHeap     *heap;   /* Heap of indexes into saved_tuples */  
    int         *heaparr;       /* Points to contents of heap */
} SampleState;

typedef struct TransducerState
{
    PlanState ps;
...
typedef struct MotionState
{
	...
	FmgrInfo *hashfunctions;	/* hash functions. */
...
typedef struct PartitionSelectorState
{
    ...
    bool exx_static_partition_propagation_called;
