...
void exx_plannedstmt_set_flags(struct PlannedStmt *stmt);
...
typedef struct Scan
{
    ...
    /*
     * XXX (ftian) : EXX EXX_IN_PG
     *
     * For dynamic scan, which is typedef-ed to be Scan.   I believe this info is some
     * how embeded in partIndex, or partIndexPrintable, but I don't know how to digging
     * it out, it is kind of twisted.
     */
    Oid         exx_part_oid;
} Scan;
...
extern int64_t get_plan_exx_share_counter(Plan *p);
extern void set_plan_exx_share_counter(Plan *p, int64_t exx_sc);
...
typedef struct ShareInputScan
{
    ...
    int64_t     exx_share_counter;              /* uniquifier */
} ShareInputScan;
...
{
    SAMPLE_RATE,
    SAMPLE_COUNT_STAGE_1,
    SAMPLE_COUNT_STAGE_2
} SampleMethod; 
...
/* -------------------------
 * EXX Transducer.
 *      The one and only evil node.
 * -------------------------
 */
typedef struct Tranducer
{
    Plan        plan;
    int         nOutCol;
    int         nInCol;
