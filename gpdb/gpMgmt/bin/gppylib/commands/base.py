...
class NakedExecutionInfo:
    ...
    def __init__(self, passwordMap):
        self.passwordMap = passwordMap

def runRemote(host, cmd, pwd=None):
    env = None
    if pwd:
        env = os.environ.copy()
        env['SSHPASSWORD'] = pwd

    # escape quote: ' is replaced with '\''
    s = cmd.split("'")
    cmd = "'\\''".join(s)

    dgcmd = "dg ssh %s %s '%s'" % ((pwd and '-P' or ''), host, cmd)
    logger.info(dgcmd)


    p = subprocess.Popen(dgcmd,
                         env = env,
                         shell = True,
    ...
    rc = p.wait()
    # CK: runRemote is supposed to return lists for out and err.
    # Do not touch!
    return (rc, out.split('\n'), err.split('\n'))
...
class NakedExecutionPasswordMap:
    ...
    def discover(self):
        ...
        for host in self.hostlist:
            ...
            # TRY NO PASSWORD
            (rc, out, err) = runRemote(host, "echo")
            if not rc:
            ...
            for passwd in self.unique_passwords:
                (rc, out, err) = runRemote(host, "echo", pwd=passwd)
                if not rc:
                    foundit = True
...
class NakedExecutionContext(LocalExecutionContext):
    ...
    def execute(self,cmd):
        (rc, out, err) = runRemote(self.targetHost, cmd.cmdStr,
                                   pwd=self.passwordMap.mapping[self.targetHost])

        if rc:
            cmd.set_results(CommandResult(rc,"","ssh command to host " + self.targetHost +
                                          " failed: " + str(err), False, False))
            return

        self.completed=True
        cmd.set_results(CommandResult(rc,out,err,self.completed, self.halt))
