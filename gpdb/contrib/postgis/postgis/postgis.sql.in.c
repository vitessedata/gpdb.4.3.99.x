...
CREATE OR REPLACE FUNCTION ST_estimated_extent(text,text,text) RETURNS box2d AS $$
BEGIN
    RAISE EXCEPTION 'Greenplum does not support ST_estimated_extent.';
END;
$$ LANGUAGE 'plpgsql';
...
CREATE OR REPLACE FUNCTION ST_MakeEnvelope(float8, float8, float8, float8 )
	RETURNS geometry
	AS 'SELECT ST_MakeEnvelope($1, $2, $3, $4, 0)'
	LANGUAGE 'sql' IMMUTABLE STRICT;
...
-----------------------------------------------------------------------
CREATE OR REPLACE FUNCTION populate_geometry_columns(use_typmod boolean)
	RETURNS text AS
$$
DECLARE
	inserted    integer;
...
CREATE OR REPLACE FUNCTION populate_geometry_columns()
	RETURNS text
	AS 'SELECT populate_geometry_columns(false)'
	LANGUAGE 'sql' IMMUTABLE STRICT;
...
-- Availability: 1.4.0
-----------------------------------------------------------------------
CREATE OR REPLACE FUNCTION populate_geometry_columns(tbl_oid oid, use_typmod boolean)
	RETURNS integer AS
$$
DECLARE
	gcs         RECORD;
...
CREATE OR REPLACE FUNCTION populate_geometry_columns(tbl_oid oid)
	RETURNS integer
	AS 'SELECT populate_geometry_columns($1, false)'
...
--
-----------------------------------------------------------------------
CREATE OR REPLACE FUNCTION AddGeometryColumn(catalog_name varchar,schema_name varchar,table_name varchar,column_name varchar,new_srid_in integer,new_type varchar,new_dim integer, use_typmod boolean)
	RETURNS text
	AS
$$
DECLARE
...
CREATE OR REPLACE FUNCTION AddGeometryColumn(catalog_name varchar,schema_name varchar,table_name varchar,column_name varchar,new_srid_in integer,new_type varchar,new_dim integer)
	RETURNS text
	AS 'SELECT AddGeometryColumn($1, $2, $3, $4, $5, $6, $7, false)'
...
--
----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION AddGeometryColumn(schema_name varchar,table_name varchar,column_name varchar,new_srid integer,new_type varchar,new_dim integer, use_typmod boolean) RETURNS text AS $$
DECLARE
	ret  text;
BEGIN
...
CREATE OR REPLACE FUNCTION AddGeometryColumn(schema_name varchar,table_name varchar,column_name varchar,new_srid integer,new_type varchar,new_dim integer)
	RETURNS text 
	AS 'SELECT AddGeometryColumn($1, $2, $3, $4, $5, $6, false)'
...
--
----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION AddGeometryColumn(table_name varchar,column_name varchar,new_srid integer,new_type varchar,new_dim integer, use_typmod boolean) RETURNS text AS $$
DECLARE
	ret  text;
BEGIN
...
CREATE OR REPLACE FUNCTION AddGeometryColumn(table_name varchar,column_name varchar,new_srid integer,new_type varchar,new_dim integer)
	RETURNS text 
	AS 'SELECT AddGeometryColumn($1, $2, $3, $4, $5, false)'
	LANGUAGE 'sql' STABLE STRICT;
...
CREATE OR REPLACE FUNCTION ST_OffsetCurve(line geometry, distance float8, params text)
       RETURNS geometry
       AS 'MODULE_PATHNAME','ST_OffsetCurve'
       LANGUAGE 'c' IMMUTABLE STRICT
       ;

CREATE OR REPLACE FUNCTION ST_OffsetCurve(line geometry, distance float8 )
       RETURNS geometry
       AS $$ SELECT ST_OffsetCurve($1, $2, '')
          $$
       LANGUAGE 'sql' IMMUTABLE STRICT;
...
-- Availability: 1.2.2
-- NOTE: On a Greenplum Database with multiple segments, the aggregate might return different answers if it
--       is called several times repeatedly, so comment this out.
...
CREATE OR REPLACE FUNCTION ST_AsSVG(geom geometry,rel int4, maxdecimaldigits int4)
	RETURNS TEXT
	AS 'MODULE_PATHNAME','LWGEOM_asSVG'
	LANGUAGE 'c' IMMUTABLE STRICT;

CREATE OR REPLACE FUNCTION ST_AsSVG(geom geometry, rel int4)
	RETURNS TEXT
	AS 'SELECT ST_AsSVG($1, $2, 15)'
	LANGUAGE 'sql' IMMUTABLE STRICT;

CREATE OR REPLACE FUNCTION ST_AsSVG(geom geometry)
	RETURNS TEXT
...
CREATE OR REPLACE FUNCTION ST_AsGML(geom geometry, maxdecimaldigits int4, options int4)
	RETURNS TEXT
	AS $$ SELECT _ST_AsGML(2, $1, $2, $3, null); $$
	LANGUAGE 'sql' IMMUTABLE STRICT;

CREATE OR REPLACE FUNCTION ST_AsGML(geom geometry, maxdecimaldigits int4)
	RETURNS TEXT
	AS 'SELECT ST_AsGML($1, $2, 0)'
	LANGUAGE 'sql' IMMUTABLE STRICT;


CREATE OR REPLACE FUNCTION ST_AsGML(geom geometry)
	RETURNS TEXT
	AS 'SELECT ST_AsGML($1, 15, 0)'
	LANGUAGE 'sql' IMMUTABLE STRICT;
...
CREATE OR REPLACE FUNCTION ST_AsGML(version int4, geom geometry, maxdecimaldigits int4, options int4, nprefix text)
	RETURNS TEXT
	AS $$ SELECT _ST_AsGML($1, $2, $3, $4,$5); $$
	LANGUAGE 'sql' IMMUTABLE;

CREATE OR REPLACE FUNCTION ST_AsGML(version int4, geom geometry, maxdecimaldigits int4, options int4)
	RETURNS TEXT
	AS $$ SELECT _ST_AsGML($1, $2, $3, $4, null); $$
	LANGUAGE 'sql' IMMUTABLE;

CREATE OR REPLACE FUNCTION ST_AsGML(version int4, geom geometry, maxdecimaldigits int4)
	RETURNS TEXT
	AS $$ SELECT _ST_AsGML($1, $2, $3, 0, null); $$
	LANGUAGE 'sql' IMMUTABLE;


CREATE OR REPLACE FUNCTION ST_AsGML(version int4, geom geometry)
	RETURNS TEXT
...
CREATE OR REPLACE FUNCTION ST_AsKML(geom geometry, maxdecimaldigits int4)
	RETURNS TEXT
	AS $$ SELECT _ST_AsKML(2, ST_Transform($1,4326), $2, null); $$
	LANGUAGE 'sql' IMMUTABLE STRICT;

CREATE OR REPLACE FUNCTION ST_AsKML(geom geometry)
	RETURNS TEXT
...
CREATE OR REPLACE FUNCTION ST_AsKML(version int4, geom geometry, maxdecimaldigits int4, nprefix text)
	RETURNS TEXT
	AS $$ SELECT _ST_AsKML($1, ST_Transform($2,4326), $3, $4); $$
	LANGUAGE 'sql' IMMUTABLE;

CREATE OR REPLACE FUNCTION ST_AsKML(version int4, geom geometry, maxdecimaldigits int4)
	RETURNS TEXT
	AS $$ SELECT _ST_AsKML($1, ST_Transform($2,4326), $3, null); $$
	LANGUAGE 'sql' IMMUTABLE;

CREATE OR REPLACE FUNCTION ST_AsKML(version int4, geom geometry)
	RETURNS TEXT
...
CREATE OR REPLACE FUNCTION ST_AsGeoJson(geom geometry, maxdecimaldigits int4, options int4)
	RETURNS TEXT
	AS $$ SELECT _ST_AsGeoJson(1, $1, $2, $3); $$
	LANGUAGE 'sql' IMMUTABLE STRICT;

CREATE OR REPLACE FUNCTION ST_AsGeoJson(geom geometry, maxdecimaldigits int4)
	RETURNS TEXT
...
-- Changed 2.0.0 to use default args and named args
CREATE OR REPLACE FUNCTION ST_AsGeoJson(gj_version int4, geom geometry, maxdecimaldigits int4, options int4)
	RETURNS TEXT
	AS $$ SELECT _ST_AsGeoJson($1, $2, $3, $4); $$
	LANGUAGE 'sql' IMMUTABLE STRICT;

CREATE OR REPLACE FUNCTION ST_AsGeoJson(gj_version int4, geom geometry, maxdecimaldigits int4)
	RETURNS TEXT
	AS $$ SELECT _ST_AsGeoJson($1, $2, $3, 0); $$
	LANGUAGE 'sql' IMMUTABLE STRICT;

CREATE OR REPLACE FUNCTION ST_AsGeoJson(gj_version int4, geom geometry)
	RETURNS TEXT
...
-- Availability 1.4.0
-- Changed 2.0.0 to use default args and named args
CREATE OR REPLACE FUNCTION ST_GeoHash(geom geometry, maxchars int4)
	RETURNS TEXT
		AS 'MODULE_PATHNAME', 'ST_GeoHash'
	LANGUAGE 'c' IMMUTABLE STRICT;


CREATE OR REPLACE FUNCTION ST_GeoHash(geom geometry)
	RETURNS TEXT
...
CREATE OR REPLACE FUNCTION postgis_type_name(geomname varchar, coord_dimension integer, use_new_name boolean) 
	RETURNS varchar
AS
$$
...
CREATE OR REPLACE FUNCTION postgis_type_name(geomname varchar, coord_dimension integer ) 
	RETURNS varchar
	AS 'SELECT postgis_type_name($1, $2, true)'
	LANGUAGE 'sql' IMMUTABLE STRICT;
...
---
-- Availability: 2.0.0
CREATE OR REPLACE FUNCTION ST_LocateBetween(Geometry geometry, FromMeasure float8, ToMeasure float8, LeftRightOffset float8)
	RETURNS geometry
	AS 'MODULE_PATHNAME', 'ST_LocateBetween'
	LANGUAGE 'c' IMMUTABLE STRICT;

CREATE OR REPLACE FUNCTION ST_LocateBetween(Geometry geometry, FromMeasure float8, ToMeasure float8)
	RETURNS geometry
...
-- Availability: 2.0.0
CREATE OR REPLACE FUNCTION ST_LocateAlong(Geometry geometry, Measure float8, LeftRightOffset float8)
	RETURNS geometry
	AS 'MODULE_PATHNAME', 'ST_LocateAlong'
	LANGUAGE 'c' IMMUTABLE STRICT;


CREATE OR REPLACE FUNCTION ST_LocateAlong(Geometry geometry, Measure float8)
	RETURNS geometry
	AS 'SELECT ST_LocateAlong($1, $2, 0.0)'
	LANGUAGE 'sql' IMMUTABLE STRICT;
...
CREATE OR REPLACE FUNCTION ST_MinimumBoundingCircle(inputgeom geometry)
	RETURNS geometry
	AS 'SELECT ST_MinimumBoundingCircle($1, 48)'
	LANGUAGE 'sql' IMMUTABLE STRICT;
...
$$
	...
	BEGIN
		...
		var_resultgeom := ST_MakeLine(array_agg(geom))
...
CREATE OR REPLACE FUNCTION ST_ConcaveHull(param_geom geometry, param_pctconvex float)
	RETURNS geometry 
	AS 'SELECT ST_ConcaveHull($1, $2, false)'
	LANGUAGE 'sql' IMMUTABLE STRICT;
...
CREATE OR REPLACE FUNCTION ST_AsX3D(geom geometry, maxdecimaldigits integer, options integer)
	RETURNS TEXT
